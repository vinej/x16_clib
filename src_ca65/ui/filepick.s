...
;
; Display-only strings (headings, tags, prompts) stay as literals: they
; only ever reach screen_blit's PETSCII fold, and the charmap's
; translation is precisely what makes them come out lower-case.
; ---------------------------------------------------------------------
...
; Cross-module: the drive commands live in storage/dos.s...
        .import         dos_chdir, dos_mkdir, dos_rmdir
        .import         dos_delete, dos_rename
; ...the mode query and the charset in video/screen.s...
        .import         screen_get_mode, screen_charset
; ...and the mouse in input/input.s.
        .import         mouse_get, mouse_hide

        .export         _x16_fp_cache
        .export         _x16_fp_filter
        .export         _x16_fp_primary
        .export         _x16_fp_style
        .export         _x16_fp_heading
        .export         _x16_fp_footing
        .export         _x16_fp_saveunder
        .export         _x16_fp_charset
        .export         _x16_fp_start_dir
        .export         _x16_fp_open
        .export         _x16_fp_resume
        .export         _x16_fp_close
        .export         _x16_fp_redraw
        .export         _x16_fp_path
        .export         _x16_fp_name
        .export         _x16_fp_dir
        .export         _x16_fp_is_primary
        .export         _x16_fp_match
        .export         _x16_fp_panel_top
        .export         _x16_fp_panel_left
        .export         _x16_fp_panel_width
        .export         _x16_fp_panel_rows
...
; ---------------------------------------------------------------------
_x16_fp_saveunder:
        sta     fp_under                ; vaddr (rightmost arg: A/X/sreg)
        stx     fp_under+1
        lda     sreg
        and     #$01
        sta     fp_underh
...
; ---------------------------------------------------------------------
_x16_fp_match:
        sta     X16_P2                  ; patterns (rightmost arg: A/X)
        stx     X16_P3
        jsr     popax                   ; name
        sta     X16_P0
        stx     X16_P1
...
filepick_s_delete:
        .byte   "delete? y/n: ", 0
filepick_s_copyfail:
        .byte   "copy failed -- press a key", 0
filepick_s_swr:
...
fp_tmp:     .res 1
fp_tmp2:    .res 1
fp_kind:    .res 1              ; an entry's type, which filepick_ent must not eat
fp_src:     .res 2              ; scratch pointers, kept out of the ZP
fp_dst:     .res 2              ; block so a library call cannot eat them
fp_pat:     .res 2
fp_ptr:     .res 2
...
fp_elen:    .res 1              ; length of the text being edited

dir_ty:     .res 1              ; the private directory walker's state

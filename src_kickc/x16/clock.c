...
__address(0x78) char* volatile x16__ck_p0;
...
__mem volatile unsigned char x16__ck_b4[4];
...
unsigned long x16_clock_get_timer(void) {
    ...
    asm {
        jsr $ffde /*RDTIM*/             // A = low, X = mid, Y = high
        sta x16__ck_b4+0
        stx x16__ck_b4+1
        sty x16__ck_b4+2
        lda #0
...
// ---------------------------------------------------------------------
void x16_clock_set_timer(__mem unsigned long jiffies) {
    unsigned long *p = (unsigned long *)x16__ck_b4;
    *p = jiffies;
    asm {
        lda x16__ck_b4+0

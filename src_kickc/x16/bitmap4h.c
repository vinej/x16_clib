...
// The operand block.
__mem volatile unsigned int x16__g4h_x;
__mem volatile unsigned int x16__g4h_y;
__mem volatile char x16__g4h_a0;         // the 20-bit SDRAM byte address
__mem volatile char x16__g4h_a1;
__mem volatile char x16__g4h_a2;
__mem volatile char x16__g4h_c;
__mem volatile char x16__g4h_off;
__mem volatile char x16__g4h_t;
__mem volatile unsigned int x16__g4h_n;

// Pattern state: raw rows plus the two colours.
__mem volatile char x16__g4h_pat[8];
__mem volatile char x16__g4h_pfg;
__mem volatile char x16__g4h_pbg;

// Blit source, pinned in the shared $78 pointer slot (see x16/zpsafe.h).
__address(0x78) const char* volatile x16__g4h_src;
...
// ---------------------------------------------------------------------
void x16__gfx4h_aim(__mem unsigned char incr) {
    asm {
        lda x16__g4h_a0
...
void x16_gfx4h_blit(unsigned int x, unsigned int y, unsigned char w,
    ...
    for (brow = 0; brow < h; brow++) {
        for (i = 0; i < w; i++) {
            ...
            ink = (unsigned char)x16__g4h_src[bidx];
            if ((i & 1) != 0) {
                ink = ink & 0x0F;
            } else {
                ink = ink >> 4;
        ...
        x16__g4h_src = x16__g4h_src + rowbytes;
    }
}
...
void x16_gfx4h_copy_wait(void) {
    asm {
    g4hcw_wait:
        lda $9f6f /*VERA2_BLIT_CTRL*/
        and #1
        bne g4hcw_wait
    }
}

void x16_gfx4h_copy(__mem unsigned long src, __mem unsigned long dst,
                   __mem unsigned long len) {
    asm {
        lda len
        sta $9f6c /*VERA2_BLIT_LEN_L*/
        lda len+1
        sta $9f6d /*VERA2_BLIT_LEN_M*/
        lda len+2
        sta $9f6e /*VERA2_BLIT_LEN_H*/
        lda src
        sta $9f62 /*VERA2_ADDR_L*/
        lda src+1
        sta $9f63 /*VERA2_ADDR_M*/
        lda src+2
        and #$0f                    // source pointer, stride +1
        sta $9f64 /*VERA2_ADDR_H*/
        lda dst
        sta $9f69 /*VERA2_BLIT_DST_L*/
        lda dst+1
        sta $9f6a /*VERA2_BLIT_DST_M*/
        lda dst+2
        and #$0f
        sta $9f6b /*VERA2_BLIT_DST_H*/
        lda #1
        sta $9f6f /*VERA2_BLIT_CTRL*/
    }
    x16_gfx4h_copy_wait();
}

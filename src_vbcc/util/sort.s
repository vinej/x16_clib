...
; vbcc argument registers: the array/base pointer rides r0/r1, the count
; r2/r3, and x16_sort's comparator pointer r4/r5. Returns: char in a.
        zpage	r0
        zpage	r1
        zpage	r2
        zpage	r3
        ...
        global	_x16_sort_u8
        global	_x16_sort_s8
        global	_x16_sort_u16
        global	_x16_sort_s16
        global	_x16_sort
...
; ...and s8 / u16 / s16, all the same shape.
; ---------------------------------------------------------------------
_x16_sort_u8:
        jsr     sort_args
        jmp     sort_u8
...
;   comparator: unsigned char cmp(__reg("r0/r1") const void *a,
;   __reg("r2/r3") const void *b), returning nonzero iff A must sort
;   AFTER B.
; ---------------------------------------------------------------------
_x16_sort:
        lda     r4                      ; cmp, behind the trampoline below
        sta     srt_ccmp
        lda     r5
        sta     srt_ccmp+1
        jsr     sort_args
...
; base (r0/r1) -> P0/P1, count (r2/r3) -> P2/P3
sort_args:
        lda     r0
        sta     X16_P0
        lda     r1
...
; element addresses to the C comparator (element A in r0/r1, element B
; in r2/r3, as the x16_sort_cmp_t prototype pins them), then turn its A
; into the carry the engine wants. The engine's P4/P5 must survive: the
; trampoline only reads them, and compiled C code touches vbcc's own
; r/btmp registers, never the library's X16_P block.
sort_c_tramp:
...
srt_base:  reserve 2               ; base address of the array
srt_count: reserve 2               ; element count
srt_size:  reserve 1               ; element size in bytes (1 or 2)
srt_cmp:   reserve 2               ; comparator routine vector
srt_i:     reserve 2               ; outer index
srt_j:     reserve 2               ; inner index
srt_key:   reserve 2               ; the element being inserted
srt_ccmp:  reserve 2               ; the C comparator behind sort_c_tramp

 ...
 *                   route to the LFO and the per-operator envelopes --
 ...
 * has no YM2151.
 *
 * Note the argument order. x16_ym_write() and x16_ym_poke() take
 * (register, value). Everything else takes (channel, ...) -- because
 * underneath, the ROM driver wants the channel in the accumulator and
 * the payload in X, which is the reverse of the register-level calls.
 * The wrappers hide that, but it is why the two families read
 * differently.
 *
 * For music rather than sound effects, look at ZSMKit.
 * =====================================================================
 */
...
** chip stayed busy.
*/
unsigned char x16_ym_write (unsigned char reg,
                                         unsigned char value);
...
unsigned char x16_ym_vol (unsigned char channel,
                                       unsigned char atten);

unsigned char x16_ym_pan (unsigned char channel,
                                       unsigned char pan);

/* Drum note 25-87. */
unsigned char x16_ym_drum (unsigned char channel,
                                        unsigned char note);

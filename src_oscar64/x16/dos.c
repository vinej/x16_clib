...
// charmap turns 'S' into PETSCII $D3. Oscar64 defaults to ASCII, but
// the bytes stay explicit hex so all four ports match.
// =====================================================================
...
unsigned char x16_dos_cmd(const char *cmd, unsigned char len) {
    ...
    dc_read:
        jsr 0xffcf                      /* CHRIN */
        cmp #0x0d                        /* the status line ends with a CR */
        beq dc_got
...
// then send. Plain C -- the compiler manages its own pointer access here.

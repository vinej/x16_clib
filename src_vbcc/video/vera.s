...
; vbcc argument registers. The (char,long) entries take inc in r0 and the
; 32-bit addr in btmp0..btmp0+3 (little-endian, only bit 16 used); fill
; takes value in r0 and count in the a/x pair.
        zpage	r0
        zpage	btmp0

        global	_x16_vera_addr0
        global	_x16_vera_addr1
        global	_x16_vera_fill
...
;
; addr is a 32-bit long, which vbcc passes in btmp0..btmp0+3 (little-
; endian): btmp0 = bits 0-7, +1 = 8-15, +2 = 16-23. Only bit 16 of the
; high half reaches the hardware (VRAM is 17-bit). inc rides in r0.
; ---------------------------------------------------------------------
_x16_vera_addr0:
        jsr     addr_marshal
        jmp     vera_set_addr0
...
; in:  btmp0.. = addr, r0 = inc
; out: A = ADDR_L, X = ADDR_M, Y = ADDR_H
addr_marshal:
        lda     btmp0                   ; addr bits 0-7
        sta     X16_T3
        lda     btmp0+1                 ; addr bits 8-15
        sta     X16_T4

        lda     btmp0+2                 ; addr bits 16-23
        and     #VERA_ADDR_H_BANK       ; ...only bit 16 exists
        sta     X16_T5                  ; ADDR_H under construction

        lda     r0                      ; A = inc | optional X16_DECR
        pha
...
; ---------------------------------------------------------------------
; void x16_vera_fill(__reg("r0") unsigned char value,
;                    __reg("a/x") unsigned int count)
;   value in r0; count in the a/x pair (A = lo, X = hi). vera_fill wants
;   A = value, X = count lo, Y = count hi.
; ---------------------------------------------------------------------
_x16_vera_fill:
        sta     X16_T3                  ; count lo (a)
        stx     X16_T4                  ; count hi (x)
        lda     r0                      ; A = value
        ldx     X16_T3
        ldy     X16_T4
        jmp     vera_fill
...
;   count in a/x (A = lo, X = hi). vera_copy wants X = lo, Y = hi.
; ---------------------------------------------------------------------
_x16_vera_copy:
        sta     X16_T3                  ; count lo
        txa

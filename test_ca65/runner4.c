/* =====================================================================
 * x16clib :: test/runner4.c -- the string library, standalone
 * =====================================================================
 * A third PRG rather than a third half of runner.c: the string modules
 * arrived as one upstream batch, and a standalone suite keeps the two
 * established runners' RAM budgets untouched. Run it with
 *
 *     .\build_ca65.ps1 -Test -Source test_ca65\runner4.c
 *
 * CHARMAPS, because every byte here is an assertion. testlib.h pulls in
 * <ascii_charmap.h>, so every C literal in THIS file compiles to its
 * ASCII byte -- which is also its ISO byte. That makes literals safe for
 * the encoding-agnostic routines (copy, find, sort...) and for nothing
 * else: the PETSCII-flavoured case/ctype tests build their inputs from
 * explicit PETSCII bytes ($41='a', $C1='A', ...), and the ISO tests
 * build theirs from explicit ISO bytes too, so no charmap -- cc65's
 * default PETSCII one or testlib's ASCII one -- can silently change
 * what is being tested.
 *
 * The ABI checks follow runner.c's principle: each test is built so the
 * answer changes if a shim pops an argument into the wrong slot -- the
 * copy test in particular goes red if source and target swap.
 * =====================================================================
 */
...
static void test_str_ltrim(void)
{
    ...
    static char lead[] = { 32, 9, 0x61, 0x62, 0 };
    static char none[] = { 0x61, 0 };
...
{
    static char tail[] = { 0x61, 0x62, 32, 9, 13, 0 };
    static char blank[] = { 32, 32, 0 };
    unsigned char ok;
...
static void test_str_trim(void)
{
    ...
    static char both[] = { 160, 32, 0x61, 0x62, 141, 10, 0 };

/* =====================================================================
 * x16clib :: test_oscar64/runner10.c -- comms: SPI, serial UART, ZiModem
 * =====================================================================
 * Standalone suite:
 *
 *      .\build_oscar64.ps1 -Test -Source test_oscar64\runner10.c
 *
 * Ported from test_ca65/runner9.c, whose coverage this matches check for
 * check. Headless reality, module by module:
 *
 ...
 * zi_hexdecode oracle.
 *
 * String literals here are ASCII because testlib.h sets
 * #pragma encoding(ascii) globally. The library's own AT strings do not
 * rely on that -- zimodem.c spells them in explicit bytes.
 * ===================================================================== */
...
**
** The readback goes through a real function call so the compiler cannot
** answer "modeled" by folding the load back into the value just stored.
*/
...
** -- this module polls). A swapped base/divisor argument cannot pass.
*/
...
static void test_ser_puts(void)
{
    ...
    fake[0] = 0;
    x16_ser_puts("HI");                 /* ASCII: testlib's encoding pragma */
...
** through ser_puts and that the module's own CRLF bytes are the right
** ones.
*/

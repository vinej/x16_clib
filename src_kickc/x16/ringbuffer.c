...
// cast literal, and __address(0x00) would make a second load block --
// which is fatal here (see input.c). bank.c does the same.

__mem volatile unsigned char x16__rng_bank;    // the HIRAM bank we own
__mem volatile unsigned char x16__rng_sav;    // the caller's RAM_BANK
__mem volatile unsigned char x16__rng_v;      // the byte in transit
__mem volatile unsigned int x16__rng_fill;     // bytes currently queued
__mem volatile unsigned int x16__rng_head;     // where the next put goes
__mem volatile unsigned int x16__rng_tail;     // one before the next get

...
volatile unsigned int x16__g2l_x;
volatile unsigned int x16__g2l_y;
volatile unsigned int x16__g2l_a;  // the byte address (fits 16 bits)
volatile char x16__g2l_c;          // colour 0-3
volatile char x16__g2l_cb;         // that colour in all four pixels
volatile char x16__g2l_off;        // pset/read's clip verdict
volatile char x16__g2l_phase;      // x & 3
volatile char x16__g2l_msk;        // RMW mask (pixels to KEEP)
volatile char x16__g2l_ink;        // RMW ink (already masked)
volatile char x16__g2l_t;

// Pattern state: 8 rows x 2 bytes, expanded once by pattern_set.
volatile char x16__g2l_pat[16];
volatile char x16__g2l_pfg;
volatile char x16__g2l_pbg;
volatile char x16__g2l_pb0;
volatile char x16__g2l_pb1;

volatile char x16__g2l_n;          // rows / columns counter
...
// ---------------------------------------------------------------------
void x16__gfx2l_aim0(unsigned char incr) {
    __asm {
        lda incr
        asl
        asl
...
void x16__gfx2l_aim1(unsigned char incr) {
    __asm {
        lda incr
        asl
        asl
...
void x16_gfx2l_init(void) {
    ...
    __asm {
        ...
        lda #0x40                        // 64 = two output pixels per input:
        sta 0x9f2a                      // VERA_DC_HSCALE (320x240 -> 640x480)

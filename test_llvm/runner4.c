/* =====================================================================
 * x16clib :: test_llvm/runner4.c -- the string library, standalone
 * =====================================================================
 * The llvm-mos port of the ca65 suite's runner4.c: same checks, same
 * names, so the two builds' results line up. Run it with
 *
 *     .\build_llvm.ps1 -Test -Source test_llvm\runner4.c
 *
 * CHARACTER SETS, because every byte here is an assertion. llvm-mos
 * does no charmap translation: every C literal in THIS file compiles
 * to its ASCII byte -- which is also its ISO byte. That makes literals
 * safe for the encoding-agnostic routines (copy, find, sort...) and
 * for nothing else: the PETSCII-flavoured case/ctype tests build their
 * inputs from explicit PETSCII bytes ($41='a', $C1='A', ...), and the
 * ISO tests build theirs from explicit ISO bytes too, so nothing can

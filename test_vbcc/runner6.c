...
/* cc65's <cx16.h> gives a struct view of VERA; vbcc has no such
** header, so the one register this suite reads is spelled out. */
...
static void test_graph_rect(void)
{
    ...
    t_check(t_vpeek(FB_VRAM(10, 20)) == 7 &&      /* top-left corner: border */
            t_vpeek(FB_VRAM(59, 49)) == 7 &&      /* bottom-right corner */
            t_vpeek(FB_VRAM(12, 22)) == 2 &&      /* interior: fill color */
            t_vpeek(FB_VRAM(60, 20)) == 0 &&      /* one past the right edge */
            t_vpeek(FB_VRAM(10, 50)) == 0,        /* one past the bottom */
            "GRAPH_RECT");
}
...
static void test_graph_line(void)
{
    ...
    t_check(t_vpeek(FB_VRAM(0, 100)) == 7 &&
            t_vpeek(FB_VRAM(5, 100)) == 7 &&
            t_vpeek(FB_VRAM(9, 100)) == 7 &&
            t_vpeek(FB_VRAM(10, 100)) == 0, "GRAPH_LINE");
}
...
static void test_graph_oval(void)
{
    ...
    t_check(t_vpeek(FB_VRAM(210, 55)) == 2 &&     /* center: filled */
            t_vpeek(FB_VRAM(199, 55)) == 0,       /* left of the box */
            "GRAPH_OVAL");
}
...
static void test_graph_move_rect(void)
{
    ...
    t_check(t_vpeek(FB_VRAM(200, 210)) == 7 &&    /* top row: border */
            t_vpeek(FB_VRAM(207, 210)) == 7 &&
            t_vpeek(FB_VRAM(200, 212)) == 7 &&    /* left column: border */
            t_vpeek(FB_VRAM(207, 212)) == 2 &&    /* interior: fill */
            t_vpeek(FB_VRAM(203, 213)) == 2 &&    /* the +1th row moved too */
            t_vpeek(FB_VRAM(208, 210)) == 0 &&    /* past the right edge */
            t_vpeek(FB_VRAM(200, 214)) == 0,      /* past the last row */
            "GRAPH_MOVE_RECT");
}
...
static void test_fb_pixels_roundtrip(void)
{
    ...
    t_check(memcmp(src, dst, 8) == 0 &&
            t_vpeek(FB_VRAM(103, 100)) == 4, "FB_PIXELS_ROUNDTRIP");
}
...
static void test_fb_set_8_pixels(void)
{
    ...
    t_check(t_vpeek(FB_VRAM(50, 50)) == 9 &&
            t_vpeek(FB_VRAM(53, 50)) == 9 &&
            t_vpeek(FB_VRAM(54, 50)) == 0 &&
            t_vpeek(FB_VRAM(57, 50)) == 0, "FB_SET_8_PIXELS");
}
...
static void test_fb_set_8_pixels_opaque(void)
{
    ...
    t_check(t_vpeek(FB_VRAM(60, 60)) == 3 &&
            t_vpeek(FB_VRAM(61, 60)) == 3 &&
            t_vpeek(FB_VRAM(62, 60)) == 4 &&
            t_vpeek(FB_VRAM(63, 60)) == 4 &&
            t_vpeek(FB_VRAM(64, 60)) == 3 &&
            t_vpeek(FB_VRAM(67, 60)) == 4, "FB_8_PIXELS_OPAQUE");
}
...
static void test_fb_fill_move(void)
{
    ...
    t_check(t_vpeek(FB_VRAM(0, 150)) == 6 &&
            t_vpeek(FB_VRAM(19, 150)) == 6 &&
            t_vpeek(FB_VRAM(20, 150)) == 0 &&
            t_vpeek(FB_VRAM(100, 151)) == 6 &&
            t_vpeek(FB_VRAM(109, 151)) == 6 &&
            t_vpeek(FB_VRAM(110, 151)) == 0, "FB_FILL_MOVE");
}
...
static void test_fb_filter(void)
{
    ...
    for (i = 0; i < 4; ++i) {
        t_vpoke(0x11, FB_VRAM(i, 160));
    }
    x16_fb_cursor_position(0, 160);
    x16_fb_filter_pixels(4, inc_filter);

    t_check(t_vpeek(FB_VRAM(0, 160)) == 0x12 &&
            t_vpeek(FB_VRAM(3, 160)) == 0x12 &&
            t_vpeek(FB_VRAM(4, 160)) == 0x00, "FB_FILTER");
}
...
static void test_fb_next_line(void)
{
    ...
    t_check(t_vpeek(FB_VRAM(5, 170)) == 8 &&
            t_vpeek(FB_VRAM(5, 171)) == 8, "FB_NEXT_LINE");
}
...
static void test_fb_palette(void)
{
    ...
    t_check(t_vpeek(0x1FA00UL + 16 * 2)     == 0x34 &&
            t_vpeek(0x1FA00UL + 16 * 2 + 1) == 0x02 &&
            t_vpeek(0x1FA00UL + 17 * 2)     == 0x21 &&
            t_vpeek(0x1FA00UL + 17 * 2 + 1) == 0x01, "FB_PALETTE");
}

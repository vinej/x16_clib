...
; vbcc argument registers: two chars land one per even register, so
; bank_alloc_init reads first from r0 and last from r2.
        zpage	r0
        zpage	r2

        global	_x16_bank_alloc_init
        global	_x16_bank_alloc
        global	_x16_bank_free
        global	_x16_bank_reserve

        section text

; ---------------------------------------------------------------------
; void x16_bank_alloc_init(__reg("r0") unsigned char first,
;                          __reg("r2") unsigned char last)

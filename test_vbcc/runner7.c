/* =====================================================================
 * x16clib :: test_vbcc/runner7.c -- the storage wave: dir, fileio, iec,
 *                              ringbuffer, stack, and buffers' counters
 * =====================================================================
 * Standalone suite: run it with
 *
 *      .\build_vbcc.ps1 -Test -Source test_ca65\runner6.c
 *
 ...
 * directory, which build_vbcc.ps1 empties before the run.
 * ===================================================================== */
...
/* Fold a PETSCII literal back to ASCII, in place. See the note in
** test_vbcc/testlib.h: -cbmascii puts every literal in PETSCII, which is
** wrong for a name the DOS parses or a name compared against one a
** directory returned. Folding twice is harmless -- after the first pass
** the bytes are $41-$5A, outside the range this touches.
*/
static void t_asc(char *s)
{
    while (*s) {
        if ((unsigned char)*s >= 0xC1 && (unsigned char)*s <= 0xDA) {
            *s = (char)((unsigned char)*s - 0x80);
        }
        ++s;
    }
}
...
{
    static char wname[] = "FIOTEST.SEQ,S,W";
    static char name[] = "FIOTEST.SEQ";
...
{
    static char rname[] = "FIOTEST.SEQ,S,R";
    unsigned char i, err, ok;
...
{
    static char name[] = "IECDEL.BIN";
    static char cmd[] = "S:IECDEL.BIN";
    static char buf[48];
...
{
    static char wname[] = "IECOUT.SEQ,S,W";
    static char name[] = "IECOUT.SEQ";
...
{
    static char small_name[] = "DIRA.BIN";
    static char big_name[] = "DIRBIG.BIN";

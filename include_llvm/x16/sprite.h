 ...
 * wrote there, not what the hardware holds. So x16_sprite_z(), which is
 * a read-modify-write, is only meaningful on a record that has already
 * been written. Call x16_sprite_init_all() once at startup to give every
 * record a known shadow.
 *
 * Sprite coordinates are 10-bit and live in the 640x480 display space,
 * NOT the 320x240 of the bitmap modes. A sprite at x=320 sits at the
 * horizontal centre whatever the screen mode.
 * =====================================================================
 */
...
/* The sprite renderer as a whole. cc65's vera_sprites_enable() is
** equivalent.
*/
...
void x16_sprite_pos (unsigned char sprite,
...
*/
void x16_sprite_image (unsigned char sprite,
                                    unsigned char mode,
                                    unsigned long addr);
...
*/
void x16_sprite_flags (unsigned char sprite,
                                    unsigned char flags);
...
void x16_sprite_size (unsigned char sprite,
                                   unsigned char width,
...
*/
void x16_sprite_setptr (unsigned char sprite,

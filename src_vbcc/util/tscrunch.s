...
; vbcc argument registers: src in r0/r1, dst in r2/r3 (see util/zx0.s).
        zpage	r0
        zpage	r1
        zpage	r2
        zpage	r3

        global	_x16_tsc_decompress
...
;   Returns one past the last output byte (in a/x).
; ---------------------------------------------------------------------
_x16_tsc_decompress:
        lda     r0
        sta     X16_P0                  ; src
        ...
        lda     r3
        sta     X16_P3
        jmp     tsc_decompress          ; already returns A = lo, X = hi

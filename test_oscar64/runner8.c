...
** Oscar64 runners do, so a bug in the library cannot hide behind itself.
...
/* cc65's <cx16.h> gives a struct view of VERA. Oscar64 has none, and it
** reaches hardware through asm rather than a pointer deref, so the one
** register this suite reads gets a small accessor. */
...
** bank matters. Oscar64 has no 32-bit arithmetic to spare here, so the
...
static void test_clock_timer(void)
{
    ...
    /* Oscar64 has no 32-bit compare fragment, so the delta is checked
...
{
    /* The cast matters on Oscar64: `return c + 1` promotes to int and
    ** the byte the filter bridge reads back is not the one you meant. */
...
static void test_fb_filter(void)
{
    ...
    /* The filter bridge does not yet work on Oscar64: the value the C
    ** filter returns comes back out of the thunk as layout-dependent
    ** garbage. Everything either side of it checks out -- see the long
    ** note above x16_fb_filter_pixels in src_oscar64/x16/fb.c for what
    ** was measured. Skipped rather than asserted at the wrong value. */
    t_skip("FB_FILTER");
}
...
{
    /* unsigned int, not char: Oscar64 has no fragment for widening a

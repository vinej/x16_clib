; =====================================================================
; x16clib :: gfx/bitmap2h.s -- 640x480x4 bitmap drawing (2bpp)
; =====================================================================
; The framebuffer is 2bpp at VRAM $00000: 4 pixels per byte packed
; MSB-first, rows of 160 bytes. A pixel byte is at y*160 + (x>>2).
; Colours are 0-3; x16_gfx2h_init() loads white/gray/gray/black into
; palette entries 0-3 and programs the mode on bare VERA registers
; (no KERNAL screen mode exists for it).
;
; x16_gfx2h_pset()/read() clip; the span/rect/line/blit primitives do
; NOT (the 8bpp module's policy). x16_gfx2h_clear() runs through the
; VERA FX 32-bit cache and needs an FX-capable VERA (47.0.2+).
;
; The routine bodies are the x16_library gfx2 module, byte for byte;
; only the C shims in front are new. Every shim copies the llvm-mos
; registers straight into the parameter block.
; =====================================================================


        .include        "macros.inc"
        .include        "x16zp.inc"

; llvm-mos argument placement, measured on the machine (see gfx/bitmap8l.s):
;   INTEGER bytes take A, then X, then single __rc bytes from __rc2 up;
;   a 16-bit int simply takes the next two byte slots. POINTERS take
;   aligned __rc PAIRS, skipping any pair already partly used.
; Returns: char in A; int in A/X.
; (import dropped: vera_fill)
; (import dropped: fx_fill)

        .globl  x16_gfx2h_init
        .globl  x16_gfx2h_clear
        .globl  x16_gfx2h_setptr
        .globl  x16_gfx2h_pset
        .globl  x16_gfx2h_read
        .globl  x16_gfx2h_hline
        .globl  x16_gfx2h_vline
        .globl  x16_gfx2h_rect
        .globl  x16_gfx2h_frame
        .globl  x16_gfx2h_line
        .globl  x16_gfx2h_pattern_set
        .globl  x16_gfx2h_pattern_rect
        .globl  x16_gfx2h_blit
        .globl  x16_gfx2h_blitm

        ; primitives the shared shape module (gfx/shapes.s) plots through
        .globl  gfx2h_pset
        .globl  gfx2h_hline
        .globl  gfx2h_read

        .section .text,"ax",@progbits

; =====================================================================
; C entry points
; =====================================================================

; void x16_gfx2h_init(void)
x16_gfx2h_init:
        jmp     gfx2h_init

; void x16_gfx2h_clear(unsigned char color)
;   color is already in A: no shim.
x16_gfx2h_clear:
        jmp     gfx2h_clear

; unsigned char x16_gfx2h_setptr(unsigned char inc,
;                               unsigned int x, unsigned int y)
; inc -> A, x -> X/__rc2, y -> __rc3/__rc4.
x16_gfx2h_setptr:
        stx     mos8(X16_P0)            ; x lo
        ldx     mos8(__rc2)
        stx     mos8(X16_P1)            ; x hi
        ldx     mos8(__rc3)
        stx     mos8(X16_P2)            ; y lo
        ldx     mos8(__rc4)
        stx     mos8(X16_P3)            ; y hi
        jmp     gfx2h_setptr             ; A = inc in, A = x & 3 out

; void x16_gfx2h_pset(unsigned int x, unsigned int y, unsigned char color)
; x -> A/X, y -> __rc2/__rc3, color -> __rc4.
x16_gfx2h_pset:
        sta     mos8(X16_P0)            ; x lo
        stx     mos8(X16_P1)            ; x hi
        lda     mos8(__rc2)
        sta     mos8(X16_P2)            ; y lo
        lda     mos8(__rc3)
        sta     mos8(X16_P3)            ; y hi
        lda     mos8(__rc4)             ; color
        jmp     gfx2h_pset

; unsigned char x16_gfx2h_read(unsigned int x, unsigned int y)
;   0-3, or $FF off screen (the body answers with carry set)
; x -> A/X, y -> __rc2/__rc3.
x16_gfx2h_read:
        sta     mos8(X16_P0)
        stx     mos8(X16_P1)
        lda     mos8(__rc2)
        sta     mos8(X16_P2)
        lda     mos8(__rc3)
        sta     mos8(X16_P3)
        jsr     gfx2h_read
        bcc     .Lread_on
        lda     #$FF
.Lread_on:
        rts

; void x16_gfx2h_hline(unsigned int x, unsigned int y,
;                     unsigned int len, unsigned char color)
; x -> A/X, y -> __rc2/__rc3, len -> __rc4/__rc5, color -> __rc6.
x16_gfx2h_hline:
        jsr     span_marshal
        lda     mos8(__rc6)             ; color
        jmp     gfx2h_hline

; void x16_gfx2h_vline(unsigned int x, unsigned int y,
;                     unsigned int len, unsigned char color)
x16_gfx2h_vline:
        jsr     span_marshal
        lda     mos8(__rc6)             ; color
        jmp     gfx2h_vline

; x, y and one more 16-bit argument into P0..P5
span_marshal:
        sta     mos8(X16_P0)            ; x lo
        stx     mos8(X16_P1)            ; x hi
        lda     mos8(__rc2)
        sta     mos8(X16_P2)            ; y lo
        lda     mos8(__rc3)
        sta     mos8(X16_P3)            ; y hi
        lda     mos8(__rc4)
        sta     mos8(X16_P4)            ; len/w lo
        lda     mos8(__rc5)
        sta     mos8(X16_P5)            ; len/w hi
        rts

; void x16_gfx2h_rect(unsigned int x, unsigned int y,
;                    unsigned int w, unsigned int h, unsigned char color)
; x -> A/X, y -> __rc2/__rc3, w -> __rc4/__rc5, h -> __rc6/__rc7,
; color -> __rc8.
x16_gfx2h_rect:
        jsr     quad_marshal
        lda     mos8(__rc8)             ; color
        jmp     gfx2h_rect

; void x16_gfx2h_frame(... same arguments ...)
x16_gfx2h_frame:
        jsr     quad_marshal
        lda     mos8(__rc8)             ; color
        jmp     gfx2h_frame

; void x16_gfx2h_line(unsigned int x0, unsigned int y0,
;                    unsigned int x1, unsigned int y1, unsigned char color)
;   the same four words land in the same four parameter slots
x16_gfx2h_line:
        jsr     quad_marshal
        lda     mos8(__rc8)             ; color
        jmp     gfx2h_line

; four 16-bit arguments into P0..P7
quad_marshal:
        jsr     span_marshal
        lda     mos8(__rc6)
        sta     mos8(X16_P6)            ; h/y1 lo
        lda     mos8(__rc7)
        sta     mos8(X16_P7)            ; h/y1 hi
        rts

; void x16_gfx2h_pattern_set(const unsigned char *pattern,
;                           unsigned char colors)
; colors (integer) -> A; pattern (pointer) -> the __rc2/__rc3 pair.
; The body wants A/X = pattern, Y = colors.
x16_gfx2h_pattern_set:
        tay                             ; colors
        lda     mos8(__rc2)
        ldx     mos8(__rc3)
        jmp     gfx2h_pattern_set

; void x16_gfx2h_pattern_rect(unsigned int x, unsigned int y,
;                            unsigned int w, unsigned int h)
x16_gfx2h_pattern_rect:
        jsr     quad_marshal
        jmp     gfx2h_pattern_rect

; void x16_gfx2h_blit(unsigned int x, unsigned int y,
;                    unsigned char wbytes, unsigned char h,
;                    const unsigned char *src, unsigned char op)
; x -> A/X, y -> __rc2/__rc3, wbytes -> __rc4, h -> __rc5,
; src -> the __rc6/__rc7 pair, op -> __rc8.
x16_gfx2h_blit:
        jsr     blit_marshal
        lda     mos8(__rc8)             ; op
        jmp     gfx2h_blit

; void x16_gfx2h_blitm(unsigned int x, unsigned int y,
;                     unsigned char h, unsigned char cols,
;                     const unsigned char *src)
; x -> A/X, y -> __rc2/__rc3, h -> __rc4, cols -> __rc5,
; src -> the __rc6/__rc7 pair. The body wants h in P4 and cols in P5 --
; the same slots blit uses for wbytes/h, so the marshal is shared.
x16_gfx2h_blitm:
        jsr     blit_marshal
        jmp     gfx2h_blitm

; x, y, two bytes, and a pointer into P0..P7
blit_marshal:
        sta     mos8(X16_P0)            ; x lo
        stx     mos8(X16_P1)            ; x hi
        lda     mos8(__rc2)
        sta     mos8(X16_P2)            ; y lo
        lda     mos8(__rc3)
        sta     mos8(X16_P3)            ; y hi
        lda     mos8(__rc4)
        sta     mos8(X16_P4)
        lda     mos8(__rc5)
        sta     mos8(X16_P5)
        lda     mos8(__rc6)
        sta     mos8(X16_P6)            ; src lo
        lda     mos8(__rc7)
        sta     mos8(X16_P7)            ; src hi
        rts

; =====================================================================
; the x16_library gfx2 module, verbatim
; =====================================================================

GFX2H_WIDTH  = 640
GFX2H_HEIGHT = 480
GFX2H_STRIDE = 160

; ---------------------------------------------------------------------
; gfx2h_init -- program the 640x480@2bpp mode on bare VERA registers.
;
; Layer 0 becomes the bitmap and is enabled; layer 1 (the text screen,
; which would overlay garbage) is disabled; sprites are left as the
; caller had them. Palette entries 0-3 get the default ramp. The
; framebuffer contents are NOT cleared -- call gfx2h_clear.
; ---------------------------------------------------------------------
gfx2h_init:
    vera_dcsel 0
    lda #$80                    ; 1:1 scale -> full 640x480
    sta VERA_DC_HSCALE
    sta VERA_DC_VSCALE
    stz VERA_DC_BORDER

    lda #(VERA_LAYER_BITMAP | VERA_LAYER_BPP_2)
    sta VERA_L0_CONFIG
    lda #$01                    ; bitmap base $00000, 640 pixels wide
    sta VERA_L0_TILEBASE
    stz VERA_L0_HSCROLL_L
    stz VERA_L0_HSCROLL_H       ; bits 3:0 = bitmap palette offset
    stz VERA_L0_VSCROLL_L
    stz VERA_L0_VSCROLL_H

    ; palette 0-3: white paper, two grays, black ink
    vera_addr 0, VRAM_PALETTE, VERA_INC_1
    ldx #0
.Lgfx2h_init_pal:
    lda bitmap2_defpal,x
    sta VERA_DATA0
    inx
    cpx #8
    bne .Lgfx2h_init_pal

    lda #VERA_VIDEO_LAYER1_EN   ; layer 1 off, layer 0 on
    trb VERA_DC_VIDEO
    lda #VERA_VIDEO_LAYER0_EN
    tsb VERA_DC_VIDEO
    rts

bitmap2_defpal:
    .byte $FF, $0F, $AA, $0A, $55, $05, $00, $00

; ---------------------------------------------------------------------
; gfx2h_clear -- fill the whole framebuffer with one colour
;   in:  A = colour (0-3)
;
; Uses the FX 32-bit cache write (~4x a CPU byte loop; measured 1.25
; frames per full screen against 5.25). Clobbers X16_P0..P4.
; ---------------------------------------------------------------------
gfx2h_clear:
    and #3
    tax
    lda bitmap2_colbyte,x
    pha
    stz mos8(X16_P0)            ; first half: $00000, 38,400 bytes
    stz mos8(X16_P1)
    stz mos8(X16_P2)
    lda #<(GFX2H_STRIDE * GFX2H_HEIGHT / 2)
    sta mos8(X16_P3)
    lda #>(GFX2H_STRIDE * GFX2H_HEIGHT / 2)
    sta mos8(X16_P4)
    pla
    pha
    jsr fx_fill
    lda #<(GFX2H_STRIDE * GFX2H_HEIGHT / 2)
    sta mos8(X16_P0)            ; second half starts at $09600
    sta mos8(X16_P3)
    lda #>(GFX2H_STRIDE * GFX2H_HEIGHT / 2)
    sta mos8(X16_P1)
    sta mos8(X16_P4)
    stz mos8(X16_P2)
    pla
    jmp fx_fill

; ---------------------------------------------------------------------
; gfx2h_setptr -- point data port 0 at the byte holding pixel (x,y)
;   in:  A = increment index (VERA_INC_*)
;        X16_P0/P1 = x, X16_P2/P3 = y
;   out: A = x & 3 (the pixel's position within the byte)
;
; y*160 = (y<<5) + (y<<5)<<2, so no multiply is needed; the result is
; 17-bit. Stepping by VERA_INC_160 then walks straight down a column.
; ---------------------------------------------------------------------
gfx2h_setptr:
    pha
    jsr bitmap2_addr_calc
    pla
    jsr bitmap2_aim0
    lda mos8(X16_P0)
    and #3
    rts

; ---------------------------------------------------------------------
; gfx2h_pset -- set one pixel, clipped
;   in:  A = colour (0-3), X16_P0/P1 = x, X16_P2/P3 = y
; ---------------------------------------------------------------------
gfx2h_pset:
    and #3
    sta g2_c
    jsr bitmap2_onscreen
    bcs .Lgfx2h_pset_off

    jsr bitmap2_addr_calc
    lda #VERA_INC_0
    jsr bitmap2_aim0

    lda mos8(X16_P0)
    and #3
    tax
    lda VERA_DATA0              ; INC_0: the read does not move the port
    and bitmap2_keep,x
    sta g2_t
    ldy g2_c
    lda bitmap2_colbyte,y
    and bitmap2_pix,x
    ora g2_t
    sta VERA_DATA0
.Lgfx2h_pset_off:
    rts

; ---------------------------------------------------------------------
; gfx2h_read -- read one pixel
;   in:  X16_P0/P1 = x, X16_P2/P3 = y
;   out: carry clear, A = colour (0-3); carry set if (x,y) is off
;        screen (A undefined)
; ---------------------------------------------------------------------
gfx2h_read:
    jsr bitmap2_onscreen
    bcs .Lgfx2h_read_roff

    jsr bitmap2_addr_calc
    lda #VERA_INC_0
    jsr bitmap2_aim0

    lda mos8(X16_P0)
    and #3
    tax
    lda VERA_DATA0
.Lgfx2h_read_shift:
    cpx #3                      ; pixel 3 sits in bits 1:0 already
    beq .Lgfx2h_read_done
    lsr
    lsr
    inx
    bra .Lgfx2h_read_shift
.Lgfx2h_read_done:
    and #3
    clc
.Lgfx2h_read_roff:
    rts

; ---------------------------------------------------------------------
; gfx2h_hline -- horizontal span (no clipping)
;   in:  A = colour (0-3), X16_P0/P1 = x, X16_P2/P3 = y,
;        X16_P4/P5 = length in pixels
;
; Head and tail partials are read-modify-write; the middle whole bytes
; are one vera_fill.
; ---------------------------------------------------------------------
gfx2h_hline:
    and #3
    tax
    lda bitmap2_colbyte,x
    sta g2_cb

    lda mos8(X16_P4)
    sta g2_n
    ora mos8(X16_P5)
    bne .Lgfx2h_hline_hgo                    ; zero length: nothing to draw
    rts
.Lgfx2h_hline_hgo:
    lda mos8(X16_P5)
    sta g2_n+1

    jsr bitmap2_addr_calc

    lda mos8(X16_P0)
    and #3
    sta g2_p                    ; phase = x & 3
    bne .Lgfx2h_hline_head
    ; phase 0: a head byte only exists when the span is shorter than
    ; one whole byte
    lda g2_n+1
    bne .Lgfx2h_hline_middle
    lda g2_n
    cmp #4
    bcs .Lgfx2h_hline_middle

.Lgfx2h_hline_head:
    ; q = last pixel of the head byte = min(3, p + n - 1)
    lda g2_n+1
    bne .Lgfx2h_hline_qmax                   ; a long span always reaches pixel 3
    clc
    lda g2_p
    adc g2_n
    bcs .Lgfx2h_hline_qmax                   ; p + n carried: certainly past pixel 3
    dec a
    cmp #4
    bcc .Lgfx2h_hline_qgot
.Lgfx2h_hline_qmax:
    lda #3
.Lgfx2h_hline_qgot:
    tay                         ; Y = q
    sec                         ; head pixel count = q - p + 1
    iny
    tya
    sbc g2_p
    sta g2_t
    ; mask = from[p] AND upto[q]
    ldx g2_p
    lda bitmap2_from,x
    dey
    and bitmap2_upto,y
    jsr bitmap2_rmw                    ; ink = colour byte through this mask

    sec                         ; n -= head pixels
    lda g2_n
    sbc g2_t
    sta g2_n
    lda g2_n+1
    sbc #0
    sta g2_n+1
    jsr bitmap2_a_inc                  ; step to the first whole byte

.Lgfx2h_hline_middle:
    ; m = n >> 2 whole bytes
    lda g2_n+1
    sta g2_m+1
    lda g2_n
    lsr g2_m+1
    ror
    lsr g2_m+1
    ror
    sta g2_m
    ora g2_m+1
    beq .Lgfx2h_hline_tail

    lda #VERA_INC_1
    jsr bitmap2_aim0
    lda g2_cb
    ldx g2_m
    ldy g2_m+1
    jsr vera_fill               ; clobbers X16_T0..T2, not g2_*

    clc                         ; addr += m
    lda g2_a0
    adc g2_m
    sta g2_a0
    lda g2_a1
    adc g2_m+1
    sta g2_a1
    lda g2_a2
    adc #0
    sta g2_a2

.Lgfx2h_hline_tail:
    lda g2_n
    and #3
    beq .Lgfx2h_hline_hdone
    tay
    dey                         ; tail covers pixels 0..n-1
    lda bitmap2_upto,y
    jsr bitmap2_rmw
.Lgfx2h_hline_hdone:
    rts

; ---------------------------------------------------------------------
; gfx2h_vline -- vertical span (no clipping)
;   in:  A = colour (0-3), X16_P0/P1 = x, X16_P2/P3 = y,
;        X16_P4/P5 = length in pixels
;
; One column of read-modify-writes: port 1 reads, port 0 writes, both
; stepping a whole row per access.
; ---------------------------------------------------------------------
gfx2h_vline:
    and #3
    tax
    lda bitmap2_colbyte,x
    sta g2_cb

    lda mos8(X16_P4)
    sta g2_n
    ora mos8(X16_P5)
    beq .Lgfx2h_vline_vdone
    lda mos8(X16_P5)
    sta g2_n+1

    jsr bitmap2_addr_calc
    lda #VERA_INC_160
    jsr bitmap2_aim1
    lda #VERA_INC_160
    jsr bitmap2_aim0

    lda mos8(X16_P0)
    and #3
    tax
    lda g2_cb
    and bitmap2_pix,x
    sta g2_ink                  ; ink and keep are loop-invariant
    lda bitmap2_keep,x
    sta g2_msk

    ldx g2_n                    ; vera_fill's page-count idiom
    ldy g2_n+1
    txa
    beq .Lgfx2h_vline_vfull                  ; low byte 0 -> exactly hi*256 rows
    iny                         ; otherwise one extra partial page
.Lgfx2h_vline_vfull:
.Lgfx2h_vline_vloop:
    lda VERA_DATA1
    and g2_msk
    ora g2_ink
    sta VERA_DATA0
    dex
    bne .Lgfx2h_vline_vloop
    dey
    bne .Lgfx2h_vline_vloop
.Lgfx2h_vline_vdone:
    rts

; ---------------------------------------------------------------------
; gfx2h_rect -- filled rectangle (no clipping)
;   in:  A = colour (0-3), X16_P0/P1 = x, X16_P2/P3 = y,
;        X16_P4/P5 = width, X16_P6/P7 = height
; ---------------------------------------------------------------------
gfx2h_rect:
    sta g2_rc
    lda mos8(X16_P4)
    sta g2_rw
    lda mos8(X16_P5)
    sta g2_rw+1
    lda mos8(X16_P6)
    sta g2_rh
    lda mos8(X16_P7)
    sta g2_rh+1
.Lgfx2h_rect_rrow:
    lda g2_rh
    ora g2_rh+1
    beq .Lgfx2h_rect_rdone
    lda g2_rw                   ; hline consumes the length: reload
    sta mos8(X16_P4)
    lda g2_rw+1
    sta mos8(X16_P5)
    lda g2_rc
    jsr gfx2h_hline              ; leaves P0..P3 alone
    inc mos8(X16_P2)            ; y += 1
    bne .Lgfx2h_rect_ry_ok
    inc mos8(X16_P3)
.Lgfx2h_rect_ry_ok:
    lda g2_rh
    bne .Lgfx2h_rect_rh_ok
    dec g2_rh+1
.Lgfx2h_rect_rh_ok:
    dec g2_rh
    bra .Lgfx2h_rect_rrow
.Lgfx2h_rect_rdone:
    rts

; ---------------------------------------------------------------------
; gfx2h_frame -- rectangle outline (no clipping)
;   same arguments as gfx2h_rect
; ---------------------------------------------------------------------
gfx2h_frame:
    sta g2_rc
    lda mos8(X16_P0)            ; private copies: the edges reuse the
    sta g2_fx                   ; parameter block as they go
    lda mos8(X16_P1)
    sta g2_fx+1
    lda mos8(X16_P2)
    sta g2_fy
    lda mos8(X16_P3)
    sta g2_fy+1
    lda mos8(X16_P4)
    sta g2_rw
    lda mos8(X16_P5)
    sta g2_rw+1
    lda mos8(X16_P6)
    sta g2_rh
    lda mos8(X16_P7)
    sta g2_rh+1

    jsr bitmap2_f_span                 ; top edge
    jsr gfx2h_hline

    jsr bitmap2_f_span                 ; bottom edge: y + h - 1
    clc
    lda g2_fy
    adc g2_rh
    sta mos8(X16_P2)
    lda g2_fy+1
    adc g2_rh+1
    sta mos8(X16_P3)
    lda mos8(X16_P2)
    bne .Lgfx2h_frame_f_nb1
    dec mos8(X16_P3)
.Lgfx2h_frame_f_nb1:
    dec mos8(X16_P2)
    lda g2_rc
    jsr gfx2h_hline

    jsr bitmap2_f_col                  ; left edge
    jsr gfx2h_vline

    jsr bitmap2_f_col                  ; right edge: x + w - 1
    clc
    lda g2_fx
    adc g2_rw
    sta mos8(X16_P0)
    lda g2_fx+1
    adc g2_rw+1
    sta mos8(X16_P1)
    lda mos8(X16_P0)
    bne .Lgfx2h_frame_f_nb2
    dec mos8(X16_P1)
.Lgfx2h_frame_f_nb2:
    dec mos8(X16_P0)
    lda g2_rc
    jmp gfx2h_vline

; x, y, width in the block, colour in A -- arguments for gfx2h_hline
bitmap2_f_span:
    lda g2_fx
    sta mos8(X16_P0)
    lda g2_fx+1
    sta mos8(X16_P1)
    lda g2_fy
    sta mos8(X16_P2)
    lda g2_fy+1
    sta mos8(X16_P3)
    lda g2_rw
    sta mos8(X16_P4)
    lda g2_rw+1
    sta mos8(X16_P5)
    lda g2_rc
    rts

; x, y, height in the block, colour in A -- arguments for gfx2h_vline
bitmap2_f_col:
    lda g2_fx
    sta mos8(X16_P0)
    lda g2_fx+1
    sta mos8(X16_P1)
    lda g2_fy
    sta mos8(X16_P2)
    lda g2_fy+1
    sta mos8(X16_P3)
    lda g2_rh
    sta mos8(X16_P4)
    lda g2_rh+1
    sta mos8(X16_P5)
    lda g2_rc
    rts

; ---------------------------------------------------------------------
; gfx2h_line -- Bresenham, any direction; plots through gfx2h_pset so
; the line clips at the screen edges
;   in:  A = colour (0-3)
;        X16_P0/P1 = x0, X16_P2/P3 = y0
;        X16_P4/P5 = x1, X16_P6/P7 = y1
; ---------------------------------------------------------------------
gfx2h_line:
    sta g2_lc
    lda mos8(X16_P0)
    sta g2_lx0
    lda mos8(X16_P1)
    sta g2_lx0+1
    lda mos8(X16_P2)
    sta g2_ly0
    lda mos8(X16_P3)
    sta g2_ly0+1
    lda mos8(X16_P4)
    sta g2_lx1
    lda mos8(X16_P5)
    sta g2_lx1+1
    lda mos8(X16_P6)
    sta g2_ly1
    lda mos8(X16_P7)
    sta g2_ly1+1

    ; dx = |x1 - x0|, sx = sign
    sec
    lda g2_lx1
    sbc g2_lx0
    sta g2_ldx
    lda g2_lx1+1
    sbc g2_lx0+1
    sta g2_ldx+1
    bpl .Lgfx2h_line_dx_pos
    sec
    lda #0
    sbc g2_ldx
    sta g2_ldx
    lda #0
    sbc g2_ldx+1
    sta g2_ldx+1
    lda #$FF
    sta g2_lsx
    sta g2_lsx+1
    bra .Lgfx2h_line_dx_done
.Lgfx2h_line_dx_pos:
    lda #$01
    sta g2_lsx
    stz g2_lsx+1
.Lgfx2h_line_dx_done:

    ; dy = -|y1 - y0|, sy = sign
    sec
    lda g2_ly1
    sbc g2_ly0
    sta g2_lt
    lda g2_ly1+1
    sbc g2_ly0+1
    sta g2_lt+1
    bpl .Lgfx2h_line_dy_pos
    sec
    lda #0
    sbc g2_lt
    sta g2_lt
    lda #0
    sbc g2_lt+1
    sta g2_lt+1
    lda #$FF
    sta g2_lsy
    sta g2_lsy+1
    bra .Lgfx2h_line_dy_done
.Lgfx2h_line_dy_pos:
    lda #$01
    sta g2_lsy
    stz g2_lsy+1
.Lgfx2h_line_dy_done:
    sec                         ; g2_ldy = -|dy|
    lda #0
    sbc g2_lt
    sta g2_ldy
    lda #0
    sbc g2_lt+1
    sta g2_ldy+1

    clc                         ; err = dx + dy
    lda g2_ldx
    adc g2_ldy
    sta g2_lerr
    lda g2_ldx+1
    adc g2_ldy+1
    sta g2_lerr+1

.Lgfx2h_line_loop:
    lda g2_lx0                  ; plot (x0, y0)
    sta mos8(X16_P0)
    lda g2_lx0+1
    sta mos8(X16_P1)
    lda g2_ly0
    sta mos8(X16_P2)
    lda g2_ly0+1
    sta mos8(X16_P3)
    lda g2_lc
    jsr gfx2h_pset

    lda g2_lx0                  ; reached the end point?
    cmp g2_lx1
    bne .Lgfx2h_line_step
    lda g2_lx0+1
    cmp g2_lx1+1
    bne .Lgfx2h_line_step
    lda g2_ly0
    cmp g2_ly1
    bne .Lgfx2h_line_step
    lda g2_ly0+1
    cmp g2_ly1+1
    bne .Lgfx2h_line_step
    rts

.Lgfx2h_line_step:
    lda g2_lerr                 ; e2 = err * 2
    asl
    sta g2_le2
    lda g2_lerr+1
    rol
    sta g2_le2+1

    ; if e2 >= dy  ->  err += dy, x0 += sx
    sec
    lda g2_le2
    sbc g2_ldy
    lda g2_le2+1
    sbc g2_ldy+1
    bvc .Lgfx2h_line_nv1
    eor #$80                    ; signed compare: fold overflow into sign
.Lgfx2h_line_nv1:
    bmi .Lgfx2h_line_skip_x
    clc
    lda g2_lerr
    adc g2_ldy
    sta g2_lerr
    lda g2_lerr+1
    adc g2_ldy+1
    sta g2_lerr+1
    clc
    lda g2_lx0
    adc g2_lsx
    sta g2_lx0
    lda g2_lx0+1
    adc g2_lsx+1
    sta g2_lx0+1
.Lgfx2h_line_skip_x:

    ; if e2 <= dx  ->  err += dx, y0 += sy
    sec
    lda g2_ldx
    sbc g2_le2
    lda g2_ldx+1
    sbc g2_le2+1
    bvc .Lgfx2h_line_nv2
    eor #$80
.Lgfx2h_line_nv2:
    bmi .Lgfx2h_line_skip_y
    clc
    lda g2_lerr
    adc g2_ldx
    sta g2_lerr
    lda g2_lerr+1
    adc g2_ldx+1
    sta g2_lerr+1
    clc
    lda g2_ly0
    adc g2_lsy
    sta g2_ly0
    lda g2_ly0+1
    adc g2_lsy+1
    sta g2_ly0+1
.Lgfx2h_line_skip_y:
    jmp .Lgfx2h_line_loop

; ---------------------------------------------------------------------
; gfx2h_pattern_set -- expand an 8x8 1bpp pattern for gfx2h_pattern_rect
;   in:  A = pattern low, X = pattern high (8 row bytes, top first;
;            bit 7 is the leftmost pixel)
;        Y = colours: (background << 2) | foreground
;
; Patterns tile from the screen origin, so each row expands to exactly
; two 2bpp bytes (16 bits); which of the pair a framebuffer byte uses
; is the parity of its address. The expansion is cached in g2_pat.
; ---------------------------------------------------------------------
gfx2h_pattern_set:
    sta mos8(X16_T6)            ; T6/T7 = pattern pointer
    stx mos8(X16_T7)
    tya
    and #3
    tax
    lda bitmap2_colbyte,x              ; replicated foreground
    sta g2_pfg
    tya
    lsr
    lsr
    and #3
    tax
    lda bitmap2_colbyte,x              ; replicated background
    sta g2_pbg

    ldx #0                      ; cache index (2 bytes per row)
    ldy #0                      ; pattern row
.Lgfx2h_pattern_set_prow:
    sty g2_t
    lda (X16_T6),y
    sta g2_pr                   ; the row's 8 bits, consumed by asl
    jsr bitmap2_p_half                 ; pixels 0-3 -> even byte
    sta g2_pat,x
    inx
    jsr bitmap2_p_half                 ; pixels 4-7 -> odd byte
    sta g2_pat,x
    inx
    ldy g2_t
    iny
    cpy #8
    bne .Lgfx2h_pattern_set_prow
    rts

; expand the next 4 bits of g2_pr (MSB first) into one 2bpp byte:
; a set bit becomes the foreground colour, a clear one the background
bitmap2_p_half:
    stz g2_t2
    ldy #0                      ; pixel 0..3 within the byte
.Lbitmap2_p_half_pbit:
    asl g2_pr
    bcs .Lbitmap2_p_half_pfg
    lda g2_pbg
    bra .Lbitmap2_p_half_pmix
.Lbitmap2_p_half_pfg:
    lda g2_pfg
.Lbitmap2_p_half_pmix:
    and bitmap2_pix,y                  ; keep just this pixel's two bits
    ora g2_t2
    sta g2_t2
    iny
    cpy #4
    bne .Lbitmap2_p_half_pbit
    lda g2_t2
    rts

; ---------------------------------------------------------------------
; gfx2h_pattern_rect -- fill a rectangle with the current pattern
;   in:  X16_P0/P1 = x, X16_P2/P3 = y, X16_P4/P5 = width,
;        X16_P6/P7 = height   (no clipping)
; ---------------------------------------------------------------------
gfx2h_pattern_rect:
    lda mos8(X16_P4)
    sta g2_rw
    lda mos8(X16_P5)
    sta g2_rw+1
    lda mos8(X16_P6)
    sta g2_rh
    lda mos8(X16_P7)
    sta g2_rh+1
.Lgfx2h_pattern_rect_yrow:
    lda g2_rh
    ora g2_rh+1
    beq .Lgfx2h_pattern_rect_ydone
    jsr bitmap2_p_row
    inc mos8(X16_P2)
    bne .Lgfx2h_pattern_rect_py_ok
    inc mos8(X16_P3)
.Lgfx2h_pattern_rect_py_ok:
    lda g2_rh
    bne .Lgfx2h_pattern_rect_ph_ok
    dec g2_rh+1
.Lgfx2h_pattern_rect_ph_ok:
    dec g2_rh
    bra .Lgfx2h_pattern_rect_yrow
.Lgfx2h_pattern_rect_ydone:
    rts

; one pattern row at (P0..P3), width g2_rw
bitmap2_p_row:
    lda g2_rw
    sta g2_n
    ora g2_rw+1
    bne .Lbitmap2_p_row_prgo
    rts
.Lbitmap2_p_row_prgo:
    lda g2_rw+1
    sta g2_n+1

    jsr bitmap2_addr_calc

    ; the row's two pattern bytes, in address-parity order
    lda mos8(X16_P2)
    and #7
    asl
    tax
    lda g2_a0
    and #1
    beq .Lbitmap2_p_row_even
    inx                         ; an odd start address uses the odd
    lda g2_pat,x                ; byte first
    sta g2_pb0
    dex
    lda g2_pat,x
    sta g2_pb1
    bra .Lbitmap2_p_row_parity_done
.Lbitmap2_p_row_even:
    lda g2_pat,x
    sta g2_pb0
    inx
    lda g2_pat,x
    sta g2_pb1
.Lbitmap2_p_row_parity_done:

    lda mos8(X16_P0)
    and #3
    sta g2_p
    bne .Lbitmap2_p_row_phead
    lda g2_n+1
    bne .Lbitmap2_p_row_pmiddle
    lda g2_n
    cmp #4
    bcs .Lbitmap2_p_row_pmiddle

.Lbitmap2_p_row_phead:
    lda g2_n+1
    bne .Lbitmap2_p_row_pqmax
    clc
    lda g2_p
    adc g2_n
    bcs .Lbitmap2_p_row_pqmax                  ; p + n carried: certainly past pixel 3
    dec a
    cmp #4
    bcc .Lbitmap2_p_row_pqgot
.Lbitmap2_p_row_pqmax:
    lda #3
.Lbitmap2_p_row_pqgot:
    tay
    sec
    iny
    tya
    sbc g2_p
    sta g2_t
    ldx g2_p
    lda bitmap2_from,x
    dey
    and bitmap2_upto,y
    tax                         ; mask in X for bitmap2_rmwp
    lda g2_pb0
    jsr bitmap2_rmwp

    sec
    lda g2_n
    sbc g2_t
    sta g2_n
    lda g2_n+1
    sbc #0
    sta g2_n+1
    jsr bitmap2_a_inc
    lda g2_pb0                  ; next byte has the other parity
    ldx g2_pb1
    sta g2_pb1
    stx g2_pb0

.Lbitmap2_p_row_pmiddle:
    lda g2_n+1
    sta g2_m+1
    lda g2_n
    lsr g2_m+1
    ror
    lsr g2_m+1
    ror
    sta g2_m
    ora g2_m+1
    beq .Lbitmap2_p_row_ptail

    lda #VERA_INC_1
    jsr bitmap2_aim0
    ldx g2_m                    ; vera_fill's page-count idiom
    ldy g2_m+1
    txa
    beq .Lbitmap2_p_row_pfull
    iny
.Lbitmap2_p_row_pfull:
.Lbitmap2_p_row_ploop:
    lda g2_pb0
    sta VERA_DATA0
    lda g2_pb0                  ; swap the parity pair
    pha
    lda g2_pb1
    sta g2_pb0
    pla
    sta g2_pb1
    dex
    bne .Lbitmap2_p_row_ploop
    dey
    bne .Lbitmap2_p_row_ploop

    clc                         ; addr += m
    lda g2_a0
    adc g2_m
    sta g2_a0
    lda g2_a1
    adc g2_m+1
    sta g2_a1
    lda g2_a2
    adc #0
    sta g2_a2

.Lbitmap2_p_row_ptail:
    lda g2_n
    and #3
    beq .Lbitmap2_p_row_prdone
    tay
    dey
    lda bitmap2_upto,y
    tax
    lda g2_pb0
    jsr bitmap2_rmwp
.Lbitmap2_p_row_prdone:
    rts

; ---------------------------------------------------------------------
; gfx2h_blit -- copy a byte-aligned image from CPU RAM into the bitmap
;   in:  A = raster op: 0 copy, 1 OR, 2 AND, 3 XOR
;        X16_P0/P1 = x (bits 1:0 ignored: byte-aligned),
;        X16_P2/P3 = y, X16_P4 = width in BYTES (4-pixel units),
;        X16_P5 = height in rows, X16_P6/P7 = source (row-major)
;
; The source pointer is X16_PTR3 -- P6/P7 double as real zero page, so
; (PTR3),y addressing costs nothing extra. No clipping.
; ---------------------------------------------------------------------
gfx2h_blit:
    and #3
    sta g2_op
    jsr bitmap2_addr_calc
    lda mos8(X16_P5)
    sta g2_h
.Lgfx2h_blit_brow:
    lda #VERA_INC_1
    jsr bitmap2_aim1                   ; ops read through port 1...
    lda #VERA_INC_1
    jsr bitmap2_aim0                   ; ...and everything writes port 0
    ldy #0
    lda g2_op
    beq .Lgfx2h_blit_bcopy
    cmp #1
    beq .Lgfx2h_blit_bor
    cmp #2
    beq .Lgfx2h_blit_band
.Lgfx2h_blit_bxor:
    lda VERA_DATA1
    eor (X16_PTR3),y
    sta VERA_DATA0
    iny
    cpy mos8(X16_P4)
    bne .Lgfx2h_blit_bxor
    bra .Lgfx2h_blit_brow_done
.Lgfx2h_blit_bcopy:
    lda (X16_PTR3),y
    sta VERA_DATA0
    iny
    cpy mos8(X16_P4)
    bne .Lgfx2h_blit_bcopy
    bra .Lgfx2h_blit_brow_done
.Lgfx2h_blit_bor:
    lda VERA_DATA1
    ora (X16_PTR3),y
    sta VERA_DATA0
    iny
    cpy mos8(X16_P4)
    bne .Lgfx2h_blit_bor
    bra .Lgfx2h_blit_brow_done
.Lgfx2h_blit_band:
    lda VERA_DATA1
    and (X16_PTR3),y
    sta VERA_DATA0
    iny
    cpy mos8(X16_P4)
    bne .Lgfx2h_blit_band
.Lgfx2h_blit_brow_done:
    clc                         ; src += width
    lda X16_PTR3
    adc mos8(X16_P4)
    sta X16_PTR3
    bcc .Lgfx2h_blit_bsrc_ok
    inc X16_PTR3+1
.Lgfx2h_blit_bsrc_ok:
    jsr bitmap2_a_row                  ; dest += one row
    dec g2_h
    bne .Lgfx2h_blit_brow
    rts

; ---------------------------------------------------------------------
; gfx2h_blitm -- masked blit of pre-shifted column-major data
;   in:  X16_P0/P1 = x (any pixel position), X16_P2/P3 = y,
;        X16_P4 = height in rows (1-127), X16_P5 = width in COLUMNS
;        (framebuffer bytes), X16_P6/P7 = source
;
; The source holds, for each of the P5 columns, P4 (mask, data) byte
; PAIRS walking down the rows: fb' = (fb AND mask) OR data. The caller
; supplies data already shifted for this x's pixel phase (x & 3) --
; pre-shifted glyph caches are the whole point: at 833 cycles per 8x8
; glyph this is what makes proportional text affordable (spike-proven;
; see the CXGEOS project). No clipping.
; ---------------------------------------------------------------------
gfx2h_blitm:
    jsr bitmap2_addr_calc
    lda mos8(X16_P5)
    sta g2_w
.Lgfx2h_blitm_mcol:
    lda #VERA_INC_160
    jsr bitmap2_aim1
    lda #VERA_INC_160
    jsr bitmap2_aim0
    ldy #0
    ldx mos8(X16_P4)
.Lgfx2h_blitm_mrow:
    lda VERA_DATA1
    and (X16_PTR3),y            ; mask byte
    iny
    ora (X16_PTR3),y            ; data byte
    iny
    sta VERA_DATA0
    dex
    bne .Lgfx2h_blitm_mrow

    clc                         ; src += 2 * height (one column)
    tya
    adc X16_PTR3
    sta X16_PTR3
    bcc .Lgfx2h_blitm_msrc_ok
    inc X16_PTR3+1
.Lgfx2h_blitm_msrc_ok:
    jsr bitmap2_a_inc                  ; dest: next byte column
    dec g2_w
    bne .Lgfx2h_blitm_mcol
    rts

; ---------------------------------------------------------------------
; module plumbing
; ---------------------------------------------------------------------

; carry clear if (P0/P1, P2/P3) is on screen
bitmap2_onscreen:
    lda mos8(X16_P1)            ; x < 640?
    cmp #>GFX2H_WIDTH
    bcc .Lbitmap2_onscreen_x_ok
    bne .Lbitmap2_onscreen_bad
    lda mos8(X16_P0)
    cmp #<GFX2H_WIDTH
    bcs .Lbitmap2_onscreen_bad
.Lbitmap2_onscreen_x_ok:
    lda mos8(X16_P3)            ; y < 480?
    cmp #>GFX2H_HEIGHT
    bcc .Lbitmap2_onscreen_ok
    bne .Lbitmap2_onscreen_bad
    lda mos8(X16_P2)
    cmp #<GFX2H_HEIGHT
    bcs .Lbitmap2_onscreen_bad
.Lbitmap2_onscreen_ok:
    clc
    rts
.Lbitmap2_onscreen_bad:
    sec
    rts

; g2_a2:a1:a0 = y*160 + (x>>2)   (from X16_P0..P3; clobbers T0..T2)
bitmap2_addr_calc:
    lda mos8(X16_P2)            ; t = y << 5
    sta g2_a0
    lda mos8(X16_P3)
    sta g2_a1
    asl g2_a0
    rol g2_a1
    asl g2_a0
    rol g2_a1
    asl g2_a0
    rol g2_a1
    asl g2_a0
    rol g2_a1
    asl g2_a0
    rol g2_a1

    lda g2_a0                   ; T2:T1:T0 = t << 2
    sta mos8(X16_T0)
    lda g2_a1
    sta mos8(X16_T1)
    stz mos8(X16_T2)
    asl mos8(X16_T0)
    rol mos8(X16_T1)
    rol mos8(X16_T2)
    asl mos8(X16_T0)
    rol mos8(X16_T1)
    rol mos8(X16_T2)

    clc                         ; y*160 = t + (t << 2)
    lda g2_a0
    adc mos8(X16_T0)
    sta g2_a0
    lda g2_a1
    adc mos8(X16_T1)
    sta g2_a1
    lda #0
    adc mos8(X16_T2)
    sta g2_a2

    lda mos8(X16_P1)            ; + x >> 2
    sta mos8(X16_T1)
    lda mos8(X16_P0)
    lsr mos8(X16_T1)
    ror
    lsr mos8(X16_T1)
    ror
    clc
    adc g2_a0
    sta g2_a0
    lda mos8(X16_T1)
    adc g2_a1
    sta g2_a1
    lda #0
    adc g2_a2
    sta g2_a2
    rts

; point port 0 (write side) at g2_a; A = increment index.
; Scratch is g2_inc, NOT g2_t: hline/pattern hold a pixel count in
; g2_t across the bitmap2_rmw call, and bitmap2_rmw aims through here.
bitmap2_aim0:
    asl
    asl
    asl
    asl
    sta g2_inc
    lda #VERA_CTRL_ADDRSEL
    trb VERA_CTRL
    lda g2_a0
    sta VERA_ADDR_L
    lda g2_a1
    sta VERA_ADDR_M
    lda g2_a2
    and #VERA_ADDR_H_BANK
    ora g2_inc
    sta VERA_ADDR_H
    rts

; point port 1 (read side) at g2_a; A = increment index
bitmap2_aim1:
    asl
    asl
    asl
    asl
    sta g2_inc
    lda #VERA_CTRL_ADDRSEL
    tsb VERA_CTRL
    lda g2_a0
    sta VERA_ADDR_L
    lda g2_a1
    sta VERA_ADDR_M
    lda g2_a2
    and #VERA_ADDR_H_BANK
    ora g2_inc
    sta VERA_ADDR_H
    rts

; read-modify-write the byte at g2_a through a pixel mask:
; fb' = (fb AND NOT mask) OR (ink AND mask). INC_0 keeps the port in
; place, so one aim serves both the read and the write.
;   bitmap2_rmw:  A = mask, ink is the solid colour byte g2_cb
;   bitmap2_rmwp: A = ink byte, X = mask (the pattern-row variant)
bitmap2_rmw:
    tax
    lda g2_cb
bitmap2_rmwp:
    sta g2_ink
    stx g2_msk
    lda #VERA_INC_0
    jsr bitmap2_aim0
    lda g2_msk
    eor #$FF
    and VERA_DATA0
    sta g2_t2
    lda g2_ink
    and g2_msk
    ora g2_t2
    sta VERA_DATA0
    rts

; g2_a += 1 (24-bit)
bitmap2_a_inc:
    inc g2_a0
    bne .Lbitmap2_a_inc_ai_done
    inc g2_a1
    bne .Lbitmap2_a_inc_ai_done
    inc g2_a2
.Lbitmap2_a_inc_ai_done:
    rts

; g2_a += one framebuffer row
bitmap2_a_row:
    clc
    lda g2_a0
    adc #GFX2H_STRIDE
    sta g2_a0
    lda g2_a1
    adc #0
    sta g2_a1
    lda g2_a2
    adc #0
    sta g2_a2
    rts

; ---------------------------------------------------------------------
; module variables (never live across a call boundary)
; ---------------------------------------------------------------------
g2_a0: .byte 0
g2_a1: .byte 0
g2_a2: .byte 0
g2_c:  .byte 0
g2_cb: .byte 0
g2_p:  .byte 0
g2_n:  .word 0
g2_m:  .word 0
g2_t:  .byte 0
g2_t2: .byte 0
g2_inc: .byte 0
g2_msk: .byte 0
g2_ink: .byte 0
g2_op: .byte 0
g2_h:  .byte 0
g2_w:  .byte 0

g2_rc: .byte 0
g2_rw: .word 0
g2_rh: .word 0
g2_fx: .word 0
g2_fy: .word 0

g2_pfg: .byte 0
g2_pbg: .byte 0
g2_pr: .byte 0
g2_pb0: .byte 0
g2_pb1: .byte 0
g2_pat: .zero  16, 0

g2_lc:  .byte 0
g2_lx0: .word 0
g2_ly0: .word 0
g2_lx1: .word 0
g2_ly1: .word 0
g2_ldx: .word 0
g2_ldy: .word 0
g2_lerr: .word 0
g2_le2: .word 0
g2_lsx: .word 0
g2_lsy: .word 0
g2_lt:  .word 0

bitmap2_colbyte:
    .byte $00, $55, $AA, $FF   ; a colour in all four pixels
bitmap2_pix:
    .byte $C0, $30, $0C, $03   ; the bits of pixel 0..3
bitmap2_keep:
    .byte $3F, $CF, $F3, $FC   ; everything but pixel 0..3
bitmap2_from:
    .byte $FF, $3F, $0F, $03   ; pixels p..3
bitmap2_upto:
    .byte $C0, $F0, $FC, $FF   ; pixels 0..q

; (end zone)

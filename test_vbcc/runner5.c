/* =====================================================================
 * x16clib :: test/runner5.c -- UTIL modules: sort, bcd, bits, number,
 *                              tscrunch
 * =====================================================================
 * A standalone suite (not part of runner.c's two halves): build and run
 * with
 *
 *      .\build_ca65.ps1 -Test -Source test_ca65\runner5.c
 *
 * Same harness contract as the main suite: testlib.h prints the
 * PASS/FAIL/DONE lines build_ca65.ps1's stdout watcher greps for.
 *
...
*/
/* The __reg() annotations must match x16_sort_cmp_t in x16/sort.h
** exactly: the sort calls this through a pointer, so a mismatch is
** silent garbage rather than a compile error.
*/
static unsigned char cmp_desc(__reg("r0/r1") const void *a,
...
/* ASCII, for the reason above. */
static const char hex_A5[]   = { 0x41, 0x35, 0x00 };
static const char hex_FF[]   = { 0x46, 0x46, 0x00 };
static const char hex_BEEF[] = { 0x42, 0x45, 0x45, 0x46, 0x00 };
static const char hex_00FF[] = { 0x30, 0x30, 0x46, 0x46, 0x00 };
static const char hex_0F[]   = { 0x30, 0x46, 0x00 };
...
static void test_u8_to_hex(void)
{
    ...
    if (!str_eq(x16_u8_to_hex(0x0F), hex_0F)) ok = 0;
    if (!str_eq(x16_u8_to_hex(0xA5), hex_A5)) ok = 0;
    if (!str_eq(x16_u8_to_hex(0xFF), hex_FF)) ok = 0;
...
static void test_u16_to_hex(void)
{
    ...
    if (!str_eq(x16_u16_to_hex(0xBEEF), hex_BEEF)) ok = 0;
    if (!str_eq(x16_u16_to_hex(0x00FF), hex_00FF)) ok = 0;
...
/* vbcc charmaps character literals to PETSCII, where the letters are
** $C1+. These modules emit and compare ASCII, so every expectation
** carrying a letter is spelled out in bytes. Digit-only strings are
** unaffected -- $30-$39 is the same in both.
*/
static const char tsc_phrase[] = {
    0x58, 0x31, 0x36, 0x4C, 0x49, 0x42, 0x2D, 0x44, 0x45, 0x43, 0x4F, 0x4D, 0x50, 0x52, 0x45, 0x53, 0x53, 0x2D, 0x54, 0x45, 0x53, 0x54, 0x21, 0x21, 0x00
};
...
static const char tsc_redge[] = { 0x52, 0x4C, 0x45, 0x2D, 0x45, 0x44, 0x47, 0x45, 0x00 };

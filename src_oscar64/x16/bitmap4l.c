...
volatile unsigned int x16__g4l_x;
volatile unsigned char x16__g4l_y;
volatile unsigned int x16__g4l_a;  // the byte address (fits 16 bits)
volatile char x16__g4l_c;          // colour 0-15
volatile char x16__g4l_off;        // pset/read's clip verdict
volatile char x16__g4l_t;

// Pattern state: the RAW 8x8 rows plus the two colours; the rotation
// happens per row at draw time (the upstream module's way).
volatile char x16__g4l_pat[8];
volatile char x16__g4l_pfg;
volatile char x16__g4l_pbg;


// Glyph cache for char/text.
volatile char x16__g4l_glyph[8];
...
// ---------------------------------------------------------------------
void x16__gfx4l_aim0(unsigned char incr) {

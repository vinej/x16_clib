...
// between save_zp and restore_zp, because compiled KickC code -- the
// dispatcher included -- uses zero-page locations the interrupted
// mainline owns mid-expression. x16/zpsafe.h confines KickC to the
// $22-$7F user window, and a callback that calls the KERNAL also
// scratches r0-r15 at $02-$21 -- so the save is $02-$7F, 126 bytes
// each way, the same window the cc65 build parked. ~2% of a frame,
// paid only when a callback is actually installed; it is what lets a
// callback call ANYTHING.
//
// The cc65 build unhooked itself at exit through a linker destructor.
// KickC has none: a program that returns to BASIC must call
// x16_irq_remove() itself.
// =====================================================================
...
// All state explicitly zeroed: KickC loads only initialised data, and
// this must not depend on leftover RAM.
__mem volatile char x16__irq_armed = 0;
__mem volatile char x16__irq_isrsnap = 0;     // ISR snapshot, current interrupt
__mem volatile char x16__irq_nframes = 0;

__mem volatile char x16__irq_line_armed = 0;
__mem volatile char x16__irq_line_fired = 0;
__mem x16_irq_handler volatile x16__irq_line_fn = 0;

__mem volatile char x16__irq_sprcol_armed = 0;
__mem volatile char x16__irq_sprcol_fired = 0;
__mem volatile char x16__irq_groups = 0;      // groups for the pending callback
__mem volatile char x16__irq_sprcol_mask = 0; // accumulated since the last read
__mem x16_sprcol_handler volatile x16__irq_sprcol_fn = 0;

// audio/pcm claims this, or nobody. An asm-level service: no zp save.
__mem x16_irq_handler volatile x16__irq_aflow_vec = 0;

__mem x16_irq_handler volatile x16__irq_old_fn = 0;

// The statically initialised trampoline targets (see the header
// comment). volatile, so KickC cannot fold them away.
__mem x16_irq_handler volatile x16__irq_handler_ptr = &x16__irq_handler;
__mem x16_irq_handler volatile x16__irq_dispatch_ptr = &x16__irq_dispatch;

__mem char x16__irq_zpsave[126];              // $02-$7F parks here
...
void x16__irq_handler(void) {
    ...
    ih_no_vsync:
        ...
        beq ih_no_line
        sta $9f27 /*VERA_ISR*/          // ack FIRST: nobody else will
    ...
    ih_no_line:
        ...
        lda x16__irq_isrsnap
        and #$04 /*VERA_IRQ_SPRCOL*/
        beq ih_no_sprcol
        sta $9f27 /*VERA_ISR*/          // ack FIRST: nobody else will
        lda x16__irq_isrsnap
        and #$f0 /*VERA_ISR_COLLISION*/ // which groups fired (7:4)
    ...
    ih_dispatch:
        ...
        // Park $02-$7F and put it back. Neither loop can be
        // interrupted: the KERNAL's stub reaches CINV with I set.
    ih_save:
        ldx #126
    ih_sv:
        lda $01,x
...
void x16_irq_install(void) {
    ...
    asm {
        ...
        sta $0315
        stz x16__irq_nframes
        lda #$01 /*VERA_IRQ_VSYNC*/
        tsb $9f26 /*VERA_IEN*/          // the KERNAL enables it anyway
        lda #1
        sta x16__irq_armed
        plp
    }
...
void x16_irq_remove(void) {
    ...
    asm {
        php
        sei
        lda #$0e                        // LINE | SPRCOL | AFLOW
        trb $9f26 /*VERA_IEN*/          // ours alone; VSYNC stays KERNAL's
        stz x16__irq_line_armed
        stz x16__irq_line_fired
        stz x16__irq_sprcol_armed
        stz x16__irq_sprcol_fired
        stz x16__irq_aflow_vec
...
// ---------------------------------------------------------------------
void x16_irq_line_install(__mem unsigned int line, __mem x16_irq_handler handler) {
    ...
    il_bit8_set:
        tsb $9f26 /*VERA_IEN*/
    il_bit8_done:
        lda #$02 /*VERA_IRQ_LINE*/
        sta $9f27 /*VERA_ISR*/          // drop any stale pending LINE
        lda #1
...
void x16_irq_line_remove(void) {
    asm {
        ...
        lda #$02 /*VERA_IRQ_LINE*/
        trb $9f26 /*VERA_IEN*/
        sta $9f27 /*VERA_ISR*/          // ack anything still pending
        stz x16__irq_line_armed
        stz x16__irq_line_fired
        plp
    }
...
// ---------------------------------------------------------------------
void x16_irq_sprcol_install(__mem x16_sprcol_handler handler) {
    x16_irq_install();
    ...
    asm {
        php
        sei
        stz x16__irq_sprcol_mask
        stz x16__irq_sprcol_fired
        lda #$04 /*VERA_IRQ_SPRCOL*/
        sta $9f27 /*VERA_ISR*/          // drop any stale pending collision
        tsb $9f26 /*VERA_IEN*/
        plp
    }
}

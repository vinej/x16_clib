...
volatile char x16__vr_t;                // ADDR_H under construction
...
void x16_vera_addr0(unsigned char incr, unsigned long addr) {
    __asm {
        lda addr+2
        and #0x01   /* ...only bit 16 exists (VERA_ADDR_H_BANK) */
        sta x16__vr_t                   /* ADDR_H under construction */
        lda incr
        and #0x0f                        /* the increment index */
        asl
        asl
        asl
...
unsigned char x16_vera_has_fx(void) {
    return __asm {
        lda 0x9f25         /* vera_dcsel 63: keep ADDRSEL, (VERA_CTRL) */
        and #0x01  /* never write bit 7 (VERA reset) (VERA_CTRL_ADDRSEL) */
        ora #0x7e                        /* 63 << 1 */
        sta 0x9f25                      /* VERA_CTRL */
        lda 0x9f29                      /* VERA_DC_VER0 */
        cmp #0x56                       /* VERA_VERSION_MAGIC 'V' */
        bne hfx_no

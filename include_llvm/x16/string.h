 ...
 *
 * The SDK's own <string.h> overlaps some of this; these match the
 * assembly library's exact semantics (lengths in a byte, capped copies
 * that always NUL-terminate, compare answering -1/0/1) and share no C
 * runtime code.
 *
 ...
 * charset, not a bug). llvm-mos does no charmap translation: a C string
 * literal is stored as its ASCII bytes, which are also its ISO bytes,
 * so literals suit the _iso routines -- PETSCII inputs must be built

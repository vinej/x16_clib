...
volatile unsigned int x16__gc_cx;
volatile char x16__gc_cy;
...
void x16_gfx_circle(unsigned int cx, unsigned char cy, unsigned char r,
    ...
    while (oy < ox) {
        ...
        oy++;
        /* explicit int steps, inherited from the KickC port (whose
        ** code generator lacked mixed-width fragments); plain C here
        */
        if (err >= 0) {
            ox--;
            t = (int)oy;
...
volatile unsigned int x16__ff_x;
volatile char x16__ff_y;
volatile unsigned int x16__ff_xl;
volatile unsigned int x16__ff_xr;
volatile unsigned int x16__ff_ax;
volatile char x16__ff_ay;
volatile char x16__ff_tgt;
volatile char x16__ff_cnt0;
volatile char x16__ff_cnt1;
volatile char x16__ff_seg;
volatile char x16__ff_sp;
volatile char x16__ff_ovf;
volatile unsigned int x16__ff_px; // scanrow's walking column
volatile char x16__ff_ny;         // the row being scanned

// The span stack, three parallel byte arrays: no slot arithmetic.
volatile char x16__ff_sxl[FF_DEPTH];
volatile char x16__ff_sxh[FF_DEPTH];
volatile char x16__ff_sy[FF_DEPTH];
...
void x16__ff_addr1(unsigned char decr) {
    __asm {
        lda decr
        beq ffa_up
        lda #0x18                        /* DECR | (INC_1 << 4) */
        jmp ffa_have
    ffa_up:
        lda #0x10                        /* INC_1 << 4 */
    ffa_have:
...
unsigned char x16_gfx_flood(unsigned int x, unsigned char y,
    ...
    __asm {
        lda 0x9f25  /*VERA_CTRL*/         /* leave ADDRSEL 0 behind */
        and #0xfe
        sta 0x9f25
    }

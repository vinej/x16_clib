...
// src_ca65/util/collide.s. What is gone is the marshalling: Oscar64
...
volatile char x16__cl_a[8];
volatile char x16__cl_b[8];
// The box pointers are indirected straight through the parameters
// (renamed pa/pb: `a` reads as the accumulator to an inline
// assembler). The cc65 build used ptr1/ptr2 for the same copies.

volatile char x16__cl_t0;
volatile char x16__cl_t1;
...
// ---------------------------------------------------------------------
unsigned char x16_collide8(unsigned char ax, unsigned char ay,
...
// ---------------------------------------------------------------------
unsigned char x16_collide16(const x16_box16 *pa, const x16_box16 *pb) {
    return __asm {
        ldy #7
    c16_copy:
        lda (pa),y

/* =====================================================================
 * x16clib :: test_llvm/runner11.c -- the file browser, driven headless
 * =====================================================================
 * STANDALONE suite:
 *
 *      .\build_llvm.ps1 -Test -Source test_ca65\runner11.c
 *
...
{
    /* llvm-mos already passes the first byte argument in A, and the "a"
    ** constraint pins it there, so the call is the whole wrapper. */
    __asm__ volatile ("jsr $FEC3" : : "a"(c) : "x", "y");
}
...
**
** fsroot's top-level files are cleared by build_llvm.ps1 before the

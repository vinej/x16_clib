/* =====================================================================
 * x16clib :: test_llvm/runner12.c -- on-machine regression suite
 * =====================================================================
 ...
 * room for BSS. So runner.c compiles into two PRGs -- test/runner13.c is
...
*/
static void test_gfx8l_clear(void)
{
...
static void test_gfx8l_pset(void)
{
...
*/
static void test_gfx8l_clip(void)
{
...
static void test_gfx8l_hline(void)
{
...
*/
static void test_gfx8l_vline(void)
{
    unsigned char i;
...
static void test_gfx8l_line(void)
{
...
static void test_gfx8l_frame(void)
{
    vpoke(0x00, PIXEL(41, 31));

    x16_gfx8l_frame(40, 30, 6, 4, 0x66);
...
*/
static void test_gfx8l_circle(void)
{
...
static void test_gfx8l_circle_r0(void)
{
    vpoke(0x00, PIXEL(100, 20));
    vpoke(0x00, PIXEL(101, 20));

    x16_gfx8l_circle(100, 20, 0, 0x92);
...
static void test_gfx8l_disc(void)
{
    x16_vera_addr0(X16_INC_1, X16_VRAM_BITMAP);
...
*/
static void test_gfx8l_ellipse(void)
{
    x16_vera_addr0(X16_INC_1, X16_VRAM_BITMAP);
    x16_vera_fill(0x00, 12800);
...
static void test_gfx8l_fellipse(void)
{
    x16_vera_addr0(X16_INC_1, X16_VRAM_BITMAP);
    x16_vera_fill(0x00, 12800);

    x16_gfx8l_fellipse(50, 20, 12, 9, 0x9B);
...
** x16_gfx8l_pset, so an outline centred off the left edge draws only its
...
*/
static void test_gfx8l_circle_clips(void)
{
    x16_vera_addr0(X16_INC_1, X16_VRAM_BITMAP);
    x16_vera_fill(0x00, 12800);

    x16_gfx8l_circle(2, 20, 10, 0x94);            /* west extreme off-screen */
...
*/
static void test_gfx8l_char(void)
{
...
/* x16_gfx8l_text advances the pen 8 pixels per character, so the second
** glyph of "AB" starts 8 to the right of the first, and the two differ.
*/
static void test_gfx8l_text(void)
{
...
** infinite loop.
*/
static void test_gfx8l_flood_noop(void)
{
    x16_vera_addr0(X16_INC_1, X16_VRAM_BITMAP);
    x16_vera_fill(0x33, 640);           /* rows 0..1 */

    t_check(x16_gfx8l_flood(10, 1, 0x33) == 1 && vpeek(PIXEL(10, 1)) == 0x33,
            "GFX8L_FLOOD_NOOP");
}
...
static void test_irq_preserves_iflag(void)
{
    ...
    x16_irq_install();
    /* One block, and REGISTER constraints. llvm-mos's "m" constraint
    ** materialises a POINTER to the object in an __rc pair, so %0 is the
    ** address's low byte rather than the object -- `sta %0` would store
    ** into __rc2. Ask for A instead and let C do the store. */
    {
        unsigned char st;
        __asm__ volatile ("php\n\tpla" : "=a"(st));
        status_after_install = st;
    }
...
{
    unsigned long jiffy0;
    unsigned int spin;

    line_fired = 0;
    jiffy0 = x16_clock_get_timer();
    ...
    } else if (x16_clock_get_timer() == jiffy0) {
...
{
    unsigned long jiffy0 = x16_clock_get_timer();
    unsigned int spin;
    int delta;
...
** clock (cc65's x16_clock_get_timer(), which reads a counter only the KERNAL's IRQ
...
static void joy_raw(unsigned char n)
{
    ...
    /* Register constraints, not "m": llvm-mos's "m" materialises a
    ** POINTER to the object in an __rc pair, so %0 would be the
    ** address's low byte and `sty %0` would store into __rc2.
    **
    ** And A is "+a", not "a". An INPUT-only register is one the compiler
    ** may assume survives the block -- it happily reused A as the next
    ** call's argument, which JOYSTICK_GET had already overwritten with
    ** joystick data. The symptom was a hang, not a wrong answer.
    */
    __asm__ volatile ("jsr $ff56" : "=y"(y), "+a"(a) : : "x");
    joy_raw_n = n;
    joy_raw_y = y;
}
...
*/
static void test_dos_rename_too_long(void)
{
    static const char oldname[] = "DOSOLD.BIN";
    static char longname[90];
    unsigned char code, i;

    for (i = 0; i < sizeof longname; i++) {
        longname[i] = 'X';
    }
    ...
    t_check(code == X16_DOS_NO_CHANNEL && x16_dos_msg()[0] == 0,
            "DOS_RENAME_TOO_LONG");
}
...
*/
static void test_dos_delete_too_long(void)
{
    static char longname[90];
    unsigned char code, i;
    ...
    code = x16_dos_delete(longname, sizeof longname);
                                ...
                                unsigned char len)
{
    ...
    /* cc65's cbm_open/cbm_write/cbm_close have no llvm-mos equivalent
    ** (its <cbm.h> exposes the KERNAL primitives, not the stdio-ish
    ** layer), so this goes through the library's own channel API -- the
    ** same calls, one level down. Secondary 1 is "write" for the DOS. */
    if (x16_fio_open_write(name, (unsigned char)strlen(name), 2,
                           X16_DEVICE_SD, 1) != 0) {
        return 1;
    }
    for (i = 0; i < len; ++i) {
        x16_fio_chrout(data[i]);
    }
...
/* x16_gfx8l_line(x0, y0, x1, y1, color): five arguments of mixed width.
...
*/
static void test_abi_gfx8l_line_argorder(void)
{
    vpoke(0x00, PIXEL(3, 7));
    ...
    t_check(vpeek(PIXEL(3, 7)) == 0x77 &&
            ...
            "ABI_GFX8L_LINE_ARGORDER");
}

/* x16_gfx8l_rect(x, y, w, h, color) with w != h. Transpose them and the
** far corner is never painted.
*/
static void test_abi_gfx8l_rect_wh(void)
{
    vpoke(0x00, PIXEL(9, 4));
    vpoke(0x00, PIXEL(4, 5));

    x16_gfx8l_rect(4, 3, 6, 2, 0x88);     /* x[4,10) y[3,5) */
...
static void test_abi_stack_balance(void)
{
    ...
    for (i = 0; i < 20; ++i) {
        ...
        x16_gfx8l_line(0, 0, 2, 2, 0);
        x16_gfx8l_rect(0, 0, 2, 2, 0);
...
static void test_g2_line(void)
{
    ...
    x16_gfx2h_line(0, 60, 7, 67, 3);     /* the 45-degree diagonal */
...
static void test_g2_pattern(void)
{
    ...
    x16_gfx2h_pattern_set(g2_pat_half, 0x03);    /* bg 0, fg 3 */
...
** tests above exercise, now plotting through gfx2h_pset/hline/read: a disc
...
** a direct fill (not x16_gfx2h_clear, which wants VERA FX).
*/
static void test_g2_disc(void)
{
    ...
    t_check(x16_gfx2h_read(40, 30) == 3 &&       /* centre */
            x16_gfx2h_read(47, 30) == 3 &&       /* inside the rim */
            x16_gfx2h_read(60, 30) == 0,         /* well outside */
            "G2_DISC");
}

/* The ellipse bound to the 2bpp module: outline extremes and a filled
** rim, through the independent gfx2h_read. */
static void test_g2_ellipse(void)
{
    x16_vera_addr0(X16_INC_1, X16_VRAM_BITMAP);
    x16_vera_fill(0x00, 160UL * 41);            /* rows 0..40 */

    x16_gfx2h_ellipse(40, 30, 15, 8, 3);

    t_check(x16_gfx2h_read(55, 30) == 3 &&       /* east */
            x16_gfx2h_read(25, 30) == 3 &&       /* west */
            x16_gfx2h_read(40, 38) == 3 &&       /* south */
            x16_gfx2h_read(40, 39) == 0 &&       /* one past the pole */
            x16_gfx2h_read(40, 30) == 0,         /* hollow */
            "G2_ELLIPSE");
}
...
static void test_g2_flood(void)
{
    ...
    x16_gfx2h_frame(20, 4, 20, 20, 3);           /* colour-3 border, 0 inside */

    t_check(x16_gfx2h_flood(25, 10, 2) == 1 &&   /* fills completely */
            x16_gfx2h_read(25, 10) == 2 &&       /* interior painted */
            x16_gfx2h_read(38, 22) == 2 &&       /* far interior corner */
            x16_gfx2h_read(20, 4) == 3 &&        /* border not crossed */
            x16_gfx2h_read(19, 10) == 0,         /* nothing leaked out */
            "G2_FLOOD");
}
...
static void test_g2_polygon(void)
{
    ...
    x16_gfx2h_fpolygon(40, 20, 12, 4, 0, 2);     /* square, filled */

    t_check(x16_gfx2h_read(40, 20) == 2 &&       /* centre filled */
            x16_gfx2h_read(40, 16) == 2 &&       /* interior */
            x16_gfx2h_read(40, 7) == 0,          /* above the north vertex */
            "G2_POLYGON");
}
...
static void test_g2_rrect(void)
{
    ...
    t_check(x16_gfx2h_read(40, 19) == 2 &&       /* interior filled */
            x16_gfx2h_read(20, 4) == 0,          /* sharp corner cut away */
            "G2_RRECT");
}
...
static void test_g2_arc(void)
{
    ...
    t_check(x16_gfx2h_read(55, 20) == 3 &&       /* east endpoint */
            x16_gfx2h_read(40, 35) == 3 &&       /* south endpoint */
            x16_gfx2h_read(25, 20) == 0 &&       /* west: not in the arc */
            x16_gfx2h_read(40, 20) == 0,         /* centre: outline */
            "G2_ARC");
}
...
static void test_g2_pie(void)
{
    ...
    t_check(x16_gfx2h_read(40, 20) == 3 &&       /* centre (fan apex) */
            x16_gfx2h_read(45, 25) == 3 &&       /* SE interior */
            x16_gfx2h_read(35, 15) == 0,         /* NW: outside the wedge */
            "G2_PIE");
}
...
static void test_g2_bezier(void)
{
    ...
    t_check(x16_gfx2h_read(20, 20) == 3 &&       /* P0 anchor */
            x16_gfx2h_read(50, 20) == 3 &&       /* P3 anchor */
            x16_gfx2h_read(35, 20) == 3 &&       /* on the line */
            x16_gfx2h_read(35, 22) == 0,         /* a 1px line: below is clear */
            "G2_BEZIER");
}
...
/* ------------------------------------------------------------------ */
/* The four engines that joined the family: bitmap4l (320x240@4bpp),
** bitmap2l (320x240@2bpp), and the VERA_2 SDRAM pair bitmap4h/8h
** (640x480). The low-res engines draw into stock VRAM, so every
** effect is vpeek-checkable; the VERA_2 pair only runs where the
** hardware answers x16_gfx*h_has().
**
...
/* Colour 7 into poisoned $00 bytes: even x, so the high nibble; the
...
static void test_g4l_vline(void)
{
    unsigned char i;
    for (i = 50; i <= 53; ++i) {
        vpoke(0x0C, (unsigned long)i * 160 + 3);
    }
...
static void test_g4l_rect(void)
{
    unsigned char i;
    for (i = 0; i < 2; ++i) {
        vpoke(0x00, P4L(4, 60 + i));
        vpoke(0x00, P4L(6, 60 + i));
    }
...
static void test_g4l_frame(void)
{
    ...
    for (i = 0; i < 3; ++i) {
        for (j = 2; j <= 4; ++j) {
            vpoke(0x00, (unsigned long)(70 + i) * 160 + j);
        }
    }
...
static void test_g4l_blit(void)
{
    ...
    vpoke(0x00, P4L(6, 110));
    vpoke(0x00, P4L(8, 110));
...
static void test_g4l_blitm(void)
{
    ...
    vpoke(0x99, P4L(6, 111));
    vpoke(0x99, P4L(8, 111));
...
{
    vpoke(0x1B, 12UL * 80);             /* pixels 0,1,2,3 left to right */
...
static void test_g2l_vline(void)
{
    unsigned char i;
    for (i = 30; i <= 34; ++i) {
        vpoke(0xFF, (unsigned long)i * 80 + 1);
    }
...
{
    vpoke(0x00, 100UL * 80 + 2);
    vpoke(0x00, 100UL * 80 + 3);
    vpoke(0x00, 101UL * 80 + 2);
    vpoke(0x00, 101UL * 80 + 3);
...
static void test_g2l_frame(void)
{
    ...
    for (i = 110; i <= 112; ++i) {
        for (j = 3; j <= 4; ++j) {
            vpoke(0x00, (unsigned long)i * 80 + j);
        }
    }
...
static void test_g2l_blit(void)
{
    ...
    vpoke(0x00, 150UL * 80 + 2);
    vpoke(0x00, 150UL * 80 + 3);
...
** suite 2 technique, minimally): the load returns ERR_FORMAT and the
** getter agrees.
*/
...
** prepend a load address. (The suite 2 half has its own copy; each PRG
** compiles only its own slice.)
*/
                                 ...
                                 unsigned char len)
{
    ...
    /* The library's channel API, as in write_raw above: llvm-mos has no
    ** cbm_open/cbm_write/cbm_close. */
    if (x16_fio_open_write(name, (unsigned char)strlen(name), 2,
                           X16_DEVICE_SD, 1) != 0) {
        return 1;
    }
...
int main(void)
{
    ...
    */
    ...
    test_abi_gfx8l_line_argorder();
    test_abi_gfx8l_rect_wh();
    test_abi_out_params();
...
#elif SUITE == 2   /* the Prog8-inspired batch */
    ...
    test_gfx8l_circle();
    test_gfx8l_circle_r0();
    test_gfx8l_disc();
    test_gfx8l_ellipse();
    test_gfx8l_fellipse();
    test_gfx8l_circle_clips();
    test_gfx8l_char();
    test_gfx8l_text();
    test_gfx8l_flood();
    test_gfx8l_flood_noop();
...
#elif SUITE == 3   /* the four new bitmap engines */

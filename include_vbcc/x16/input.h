 ...
 * =====================================================================
 * Thin wrappers over the KERNAL: the X16's own calls, with no driver to
 * install and the SNES pad's full button set exposed.
 * =====================================================================
 */
...
/* `joy` is 0 for the keyboard, 1-4 for gamepads. Returns the button bits,
** active low. *present becomes 1 or 0. */
unsigned int x16_joy_get(__reg("r0") unsigned char joy,
                         __reg("r2/r3") unsigned char *present);
...
void x16_mouse_show(__reg("a") unsigned char cursor);
void x16_mouse_hide(void);

/* Returns the button mask; writes the position through the pointers. */
unsigned char x16_mouse_get(__reg("r0/r1") unsigned int *x, __reg("r2/r3") unsigned int *y);

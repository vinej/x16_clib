...
; vbcc argument registers, plus the C soft-stack pointer for fx_line's
; fifth argument. 16-bit ints/pointers ride register PAIRS; a 32-bit long
; rides btmp0 (and a second long btmp1, as in fx_copy).
        zpage	r0
        zpage	r1
        zpage	r2
        zpage	r3
...
;   fx_mult reads P0/P1=a, P2/P3=b, writes P4..P7. The long return leaves
;   in btmp0..btmp0+3.
_x16_fx_mult:
        lda     r0
        sta     X16_P0
...
;                  unsigned long addr)
;   value in r0, count in r2/r3, addr (long) in btmp0..btmp0+2.
;   fx_fill wants P0/P1/P2 = addr, P3/P4 = count, A = value.
_x16_fx_fill:
        lda     btmp0
...
;   count in r0/r1, addr (long) in btmp0. fx_fill wants P0/P1/P2 = addr,
;   P3/P4 = count, A = value (0).
; ---------------------------------------------------------------------
_x16_fx_clear:
        lda     btmp0
        sta     X16_P0
...
;                  __reg("r0/r1") unsigned int count)
;   Two longs: src in btmp0..btmp0+2, dst in btmp1..btmp1+2 (only bit 16 of
;   each high half exists); count in r0/r1. fx_copy wants P0/P1 = src lo/mid,
;   P2 = src bank, P3/P4 = dst lo/mid, P5 = dst bank, P6/P7 = count.
_x16_fx_copy:
        lda     btmp0
...
;                  unsigned char color)
;   Five arguments: color (the 5th) rides the C soft stack at (sp)+0.
;   fx_line wants P0/P1 = x0, P2 = y0, P3/P4 = x1, P5 = y1, P6 = color.
_x16_fx_line:
        lda     r0
        sta     X16_P0                  ; x0
        lda     r1
...
;   Each vertex is a 3-byte struct { unsigned int x; unsigned char y; };
;   copy each through X16_PTR2. color rides r6.
_x16_fx_triangle:
        lda     r6
        sta     tri_color               ; colour

        lda     r4
        sta     X16_PTR2                ; vertex c
        lda     r5
        sta     X16_PTR2+1
        ldy     #2
.copy_c:
...
fxt_n2:    reserve 1
fxt_swap:  reserve 1
fxt_xl:    reserve 1                       ; encoded increments for the two slots
fxt_xh:    reserve 1
fxt_yl:    reserve 1
fxt_yh:    reserve 1
fxt_px:    reserve 2                       ; starting x of each edge
fxt_py:    reserve 2
fxt_a_l:   reserve 1                       ; the long edge, parked
fxt_a_h:   reserve 1
fxt_a_sgn: reserve 1
fxt_a_mag: reserve 3
fxt_rows:  reserve 1
